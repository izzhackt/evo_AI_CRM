...
V1/V2 are historical and rollback inputs, not parallel products. The accepted
2026-09-17 domain cutover uses `https://crm.evoadmissions.com` for staff and
`https://app.evoadmissions.com` for Students over that same app. This supersedes
custom-domain deferral; the launch-plan receipt and
[domain runbook](deploy/README.md) record exact release and live verification.
Real Student sign-in and email delivery are not claimed. Preserve public Host,
host-only cookies and audience-specific exact callbacks; do not introduce a
shared wildcard session or second runtime. After this accepted cutover, the old
