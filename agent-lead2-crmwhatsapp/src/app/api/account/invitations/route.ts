...
//   4. Last-resort planned inbox-domain fallback. Only hit if the
...
//   on the list falls through to the configured EVO Inbox fallback with a
...
//
// The fallback is the planned companion inbox domain. This is not a
// deployment claim; production still needs the later DNS/Caddy/VPS slice.
...
function getBaseUrl(request: Request): string {
  ...
  if (allowList && (forwardedHost || host)) {
    ...
    console.warn(
      "[POST /api/account/invitations] could not derive base URL from request; falling back to planned inbox domain",
    );
  }
  return "https://inbox.evoadmissions.com";
}
