...
const ROLE_LABEL_KEY: Record<PeekOk['role'], TranslationKey> = {
  admin: 'common.admin',
  agent: 'common.agent',
...
const FAIL_COPY_KEY: Record<PeekFail['reason'], { title: TranslationKey; body: TranslationKey }> = {
  ...
  used: {
    title: 'auth.join.fail.used.title',
    body: 'auth.join.fail.used.body',
  },
  expired: {
    title: 'auth.join.fail.expired.title',
    body: 'auth.join.fail.expired.body',
  },
  server_error: {
    title: 'auth.join.fail.server.title',
    body: 'auth.join.fail.server.body',
  },
};
...
export default function JoinPage() {
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      toast.success(t('auth.join.welcome'));
            ...
            <DialogFooter className="bg-popover border-border">
              ...
              >
                {signingOut ? (
                  <>
                    <Loader2 className="size-4 animate-spin" />
                    {t('auth.join.signingOut')}
                  </>
                ) : (
                  t('auth.join.signOutDifferent')
                )}
              </Button>
            </DialogFooter>
          </DialogContent>
