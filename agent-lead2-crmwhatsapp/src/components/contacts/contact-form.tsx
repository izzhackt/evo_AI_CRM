...
export function ContactForm({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!isEdit && dupMatch?.exact) {
      toast.error(t('contacts.form.duplicateExact'));
      return;
    }
    ...
    try {
      ...
      if (!user) throw new Error(t('auth.error.sessionRequired'));
      if (!accountId) throw new Error(t('pipelines.profileNoAccount'));
      ...
      toast.success(
        t(isEdit ? 'contacts.form.updated' : 'contacts.form.created'),
      );
      onOpenChange(false);
      onSaved();
    } catch (err: unknown) {
      ...
      const message =
        err instanceof Error ? err.message : t('contacts.form.saveFailed');
      toast.error(message);
    } finally {
      setSaving(false);
    }
      ...
      <DialogContent className="bg-popover border-border text-popover-foreground sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">
            {t(isEdit ? 'contacts.form.editTitle' : 'contacts.form.addTitle')}
          </DialogTitle>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {dupMatch ? (
              ...
              >
                ...
                <div className="space-y-1">
                  <p>
                    {dupMatch.exact
                      ? t('contacts.form.duplicateExact')
                      : t('contacts.form.duplicateSimilar')}
                  </p>
                  {onViewExisting && (
                    <button
                      type="button"
          ...
          <div className="space-y-2">
            <Label className="text-muted-foreground">{t('inbox.tags')}</Label>
            {loadingTags ? (
