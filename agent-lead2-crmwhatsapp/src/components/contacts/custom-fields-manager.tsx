      ...
      <DialogContent className="border-border bg-popover text-popover-foreground sm:max-w-md">
        <DialogHeader>
          <DialogTitle className="text-popover-foreground">
            {t('settings.fields.customTitle')}
          </DialogTitle>
          <DialogDescription className="text-muted-foreground">
            {t('settings.fields.customDescription')}
          </DialogDescription>
        </DialogHeader>
        <CustomFieldsPanel />
      </DialogContent>
...
export function CustomFieldsPanel() {
  ...
  async function handleCreate() {
    ...
    if (isDuplicate(name)) {
      toast.error(t('settings.fields.duplicateField', { name }));
      return;
    }
    ...
    if (error) {
      toast.error(t('settings.fields.createFieldFailed'));
      return;
    }
    toast.success(t('settings.fields.fieldCreated', { name }));
    setNewName('');
    await fetchFields();
  }
  ...
  async function handleRename(
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('settings.fields.renameFieldFailed'));
      return false;
    }
    await fetchFields();
    return true;
  ...
  async function handleDelete(field: CustomField) {
    if (
      !window.confirm(
        t('settings.fields.deleteFieldConfirm', { name: field.field_name })
      )
    ) {
      return;
    }
    ...
    setBusyId(null);
    if (error) {
      toast.error(t('settings.fields.deleteFieldFailed'));
      return;
    }
    toast.success(t('settings.fields.fieldDeleted', { name: field.field_name }));
    await fetchFields();
  }
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-2">
        <Input
          ...
          placeholder={t('settings.fields.newFieldPlaceholder')}
      ...
      <div className="max-h-72 overflow-y-auto rounded-md border border-border">
        {loading ? (
          ...
          <p className="py-8 text-center text-sm text-muted-foreground">
            {t('settings.fields.noCustomFields')}
          </p>
        ) : (
  ...
  return (
    <li className="flex items-center gap-2 px-3 py-2">
      <Input
        ...
        aria-label={t('settings.fields.renameField', {
      ...
      <Button
        ...
        title={t('settings.fields.deleteField')}
