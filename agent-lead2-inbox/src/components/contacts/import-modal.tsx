...
export function ImportModal({
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (rows.length === 0) {
      toast.error(t('contacts.import.noValidRows'));
      setParsedRows([]);
      setHasTagsColumn(false);
      setHasCompanyColumn(false);
      setTagColorByKey(new Map());
  ...
  async function handleImport() {
    ...
    try {
      ...
      if (!user) throw new Error(t('contacts.import.noAuth'));
      if (!accountId) throw new Error(t('contacts.import.noAccount'));
      ...
      try {
        ...
        toast.warning(t('contacts.import.tagAssignFailed'));
      }
      ...
      if (imported > 0) {
        toast.success(
          imported === 1
            ? t('contacts.import.contactImported')
            : t('contacts.import.contactsImported', { count: imported })
        );
        onImported();
      }
      if (tagsAssigned > 0) {
        toast.success(
          tagsAssigned === 1
            ? t('contacts.import.tagAssigned')
            : t('contacts.import.tagsAssigned', { count: tagsAssigned })
        );
      }
      if (skippedNames.length > 0) {
        ...
        const more =
          skippedNames.length > 3
            ? t('contacts.import.moreNames', {
                count: skippedNames.length - 3,
              })
            : '';
        toast.info(
          t('contacts.import.unknownTagsSkipped', {
            names: `${sample}${more}`,
          })
        );
      }
      if (skipped > 0) {
        toast.info(
          skipped === 1
            ? t('contacts.import.duplicateSkipped')
            : t('contacts.import.duplicatesSkipped', { count: skipped })
        );
      }
      if (failed > 0) {
        toast.error(
          failed === 1
            ? t('contacts.import.contactFailed')
            : t('contacts.import.contactsFailed', { count: failed })
        );
      }
    } catch (err: unknown) {
      const message =
        err instanceof Error ? err.message : t('contacts.import.importFailed');
      toast.error(message);
    } finally {
      setImporting(false);
    }
        ...
        <div className="border-border/80 shrink-0 space-y-4 border-b px-6 pt-6 pb-5">
          <DialogHeader className="gap-1.5">
            <DialogTitle className="text-popover-foreground text-lg">
              {t('contacts.import.leadTitle')}
            </DialogTitle>
            <DialogDescription className="text-muted-foreground leading-relaxed">
              {t('contacts.import.detailedDescription')}
            </DialogDescription>
          </DialogHeader>
            ...
            {file ? (
              <>
                ...
                <span className="bg-muted text-muted-foreground rounded-full px-2.5 py-0.5 text-[11px] font-medium">
                  {parsedRows.length === 1
                    ? t('contacts.import.rowReady')
                    : t('contacts.import.rowsReady', {
              ...
              <>
                ...
                <p className="text-muted-foreground text-sm">
                  {t('contacts.import.chooseCsv')}
                </p>
                <p className="text-muted-foreground text-[11px]">
                  {t('contacts.import.csvLimit')}
                </p>
              </>
            )}
          </div>
                  ...
                  <table className="w-full min-w-[32rem] text-xs">
                    <thead>
                      <tr className="border-border bg-background/60 border-b">
                        ...
                        {previewHasCompany && (
                          <th className="text-muted-foreground px-3 py-2 text-left font-medium whitespace-nowrap">
                            {t('contacts.import.company')}
                          </th>
                        )}
                        {previewHasTags && (
                          <th className="text-muted-foreground px-3 py-2 text-left font-medium whitespace-nowrap">
                            {t('contacts.import.tags')}
                          </th>
                        )}
                      </tr>
                    </thead>
              ...
              {parsedRows.length > PREVIEW_LIMIT && (
                <p className="text-muted-foreground text-center text-[11px]">
                  {parsedRows.length - PREVIEW_LIMIT === 1
                    ? t('contacts.import.moreRow')
                    : t('contacts.import.moreRows', {
                ...
                {result.tagsAssigned > 0 && (
                  <div className="flex items-center gap-1.5 text-sm text-cyan-400">
                    ...
                    {result.tagsAssigned === 1
                      ? t('contacts.import.tagAssignedShort')
                      : t('contacts.import.tagsAssignedShort', {
        ...
        <DialogFooter className="border-border/80 bg-background/50 mt-0 shrink-0 gap-2 border-t px-6 py-4 sm:justify-end">
          ...
          {!result && (
            ...
            >
              ...
              {parsedRows.length === 0
                ? t('contacts.import.buttonEmpty')
                : parsedRows.length === 1
                  ? t('contacts.import.buttonOne')
