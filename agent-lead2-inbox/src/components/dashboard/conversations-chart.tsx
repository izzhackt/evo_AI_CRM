      ...
      {hovered && hover !== null && (
        ...
        >
          <div className="font-medium text-popover-foreground">
            {longDayLabel(hovered.day, locale)}
          </div>
          <div className="mt-1 flex flex-col gap-0.5">
            <span className="flex items-center gap-1.5 text-blue-300">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-blue-500" />
              {t('dashboard.conversations.incomingCount', { count: hovered.incoming })}
            </span>
            <span className="flex items-center gap-1.5 text-primary">
              <span className="inline-block h-1.5 w-1.5 rounded-full bg-primary" />
              {t('dashboard.conversations.outgoingCount', { count: hovered.outgoing })}
            </span>
          </div>
        </div>
      )}
...
function shortDayLabel(key: string, locale: string): string {
  ...
  return date.toLocaleDateString(locale === 'ru' ? 'ru-RU' : undefined, { month: 'short', day: 'numeric' })
}

function longDayLabel(key: string, locale: string): string {
  const [y, m, d] = key.split('-').map(Number)
  const date = new Date(y, m - 1, d)
  return date.toLocaleDateString(locale === 'ru' ? 'ru-RU' : undefined, { weekday: 'short', month: 'short', day: 'numeric' })
}
