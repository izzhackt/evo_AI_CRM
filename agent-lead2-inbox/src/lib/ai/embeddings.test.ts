...
describe('embedTexts', () => {
  it('returns [] and makes no request for empty input', async () => {
    ...
    expect(
      await embedTexts({ provider: 'openai', apiKey: 'sk-x' }, [])
    ).toEqual([]);
  ...
  it('embeds a single batch and sends the key', async () => {
    ...
    const out = await embedTexts({ provider: 'openai', apiKey: 'sk-x' }, [
      'a',
      'b',
      'c',
    ]);
  ...
  it('calls Gemini embedContent with a 1536-dim retrieval query prompt', async () => {
    ...
    expect(
      (opts as { headers: Record<string, string> }).headers['x-goog-api-key']
    ).toBe('AIza-test');
