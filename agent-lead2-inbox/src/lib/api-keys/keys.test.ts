...
describe('hashApiKey', () => {
  it('is deterministic', () => {
    expect(hashApiKey('evoinbox_live_abc')).toBe(
      hashApiKey('evoinbox_live_abc')
    );
  });

  it('differs for different inputs', () => {
    expect(hashApiKey('evoinbox_live_abc')).not.toBe(
      hashApiKey('evoinbox_live_abd')
    );
  });
});
...
describe('timingSafeHexEqual', () => {
  it('is true for identical digests', () => {
    const h = hashApiKey('evoinbox_live_xyz');
