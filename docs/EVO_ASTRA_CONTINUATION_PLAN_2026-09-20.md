...
**Зависимости.** П.3 — миграция 207+ (номера распределяет Astra как координатор после свежей сверки); п.6 отложен с AST-2; п.7 — записанное решение до реализации; остальные независимы.
