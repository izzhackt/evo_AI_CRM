...
Дата: 20 сентября 2026. Статус: согласованный объём продолжения после закрытия портального плана; исполнение ведёт GPT-6 Astra. Основа — origin/main `49f071488972445b2c4fded63a1e6cba7d8ab0bb` (включает финальную ведомость и handover, PR #917, смержённый PR #918 с коррекцией сортировки и canonical search и смержённый PR #919 с защитой несохранённых правок и selected canonical exports); сверено на момент составления (2026-09-20 00:27 UTC).
...
Твоя работа состоит из пяти блоков: завершение собственного KB-плана (действующий контракт, приоритет №1), живой E2E-прогон новых портальных флоу, контентная волна 2, подготовка агентской стороны App Store и очередь UX/тех-долгов портала. Каждый блок описан в разделе 5 с составом, зависимостями и критериями завершения.
...
```mermaid
flowchart LR
  A1[AST-1: завершение KB] --> A2[AST-2: живой E2E с владельцем]
  A2 --> A3[AST-3: контентная волна 2]
  A2 --> A4[AST-4: App Store readiness]
  A5[AST-5: очередь долгов] -.фоном между блоками.-> A3
  A5 -.-> A4
```

Порядок рекомендуемый, и вот его обоснование. **AST-1 первым**: это единственный блок с уже действующим контрактом и наполовину применённым состоянием — схема в проде, ключ установлен, перенос запущен, выпуск #918 ожидается; бросать перенос на середине означает оставить базу в худшем из состояний (две системы, ни одна не полная). **AST-2 вторым**: это самый большой honesty-gap ведомости — все write-флоу с волны 5 не проверены против production; контент (AST-3) и стор-материалы (AST-4) честнее строить на доказанных путях, а прогон требует координации с владельцем (живая сессия симулятора, staff-одобрение), поэтому договариваться о ней нужно рано. **AST-3 и AST-4 параллельно после**: они независимы друг от друга; AST-4 упирается в владельческие Apple-шаги, поэтому агентскую часть стоит подготовить заранее, не дожидаясь их. **AST-5 — фоновая очередь**: малые изолированные PR, берутся между крупными блоками, каждый по полной дисциплине. Внутри этого порядка Astra вправе параллелить работу, если зависимости и дисциплина раздела 7 соблюдены.
...
### AST-2 — живой E2E-прогон новых портальных флоу
...
Координация с владельцем обязательна: iOS-часть требует залогиненного устройства/симулятора — договорись о живой сессии симулятора; staff-одобрение регистрации выполняет владелец/сотрудник по существующему процессу.
...
**Зависимости.** Движок 198 готов; желательно после AST-2 (write-пути раннеров доказаны до массового контента) — мягкая, не жёсткая зависимость; хранилище аудио — решение по схеме за Astra как координатором.
...
### AST-4 — App Store readiness агентской стороны

**Зачем.** iOS-код в main с 139/139 тестов, но дистрибуции ноль — это единственный блокер доставки всего iPhone-клиента пользователям. Агентская часть готовится заранее, чтобы после владельческих Apple-шагов не осталось ничего, кроме сабмита.
...
**Зависимости.** Скриншоты и review notes честнее после AST-2; privacy manifest — только от фактического кода; остальное независимо.
...
### AST-5 — очередь UX/тех-долгов портала

**Зачем.** Ведомость и review-квитанции оставили список малых конкретных долгов; они закрываются изолированными PR по полной дисциплине, не смешиваясь с крупными блоками.

**Состав** (каждый пункт — отдельный PR либо явная запись отказа с причиной в `PLAN_CHANGES.md`):

1. iOS: отрисовка операционной стадии в `MyAdmissionView` — декодируется (`AdmissionModels.swift:32`), но не отображается; веб показывает с #903.
2. iOS: паритет «Главной» с веб PORT-9c — продолжить урок/тест из состояния попытки, избранное с ближайшими интейками, карточка статуса анкеты.
3. «Новое в каталоге» на веб-Главной: каталог не даёт сигнала first-published (`publishedAt` = `reviewed_at` текущей версии, миграция 148 — показал бы «недавно отредактированные»); нужна серверная работа и **решение по схеме — миграция 207+**.
4. Tier-derivation cleanup: `(portal)/layout.tsx:21–24` всё ещё выводит `accessTier` локально из caseState, тогда как authority уже отдаёт `actor.accessTier` (используется `home/page.tsx:38–39` после #907) — консолидировать на один источник.
5. Порт `PortalNotificationUpdates.tsx` — последнего v3-компонента, назначенного к переносу «следующим slices» (`PLAN_CHANGES.md:31682–31684`), — в мир «Атласа»; сознательно оставленный `CaseHelpWorkspace` (v3/profile на `/portal`, PORT-5d) в это назначение не входит и трогается только отдельным решением.
6. E2E-покрытие a11y-фокус фиксов #896: focus-transfer, radiogroup в `exercises.tsx`, клавиатурная прокрутка FavoritesView, фокус диалога MapCanvas (follow-up, названный ревьюером).
...
11. Отдельный путь подтверждения `/apply` (SMTP-конфиг сам его не включает, `EVO_LAUNCH_PLAN.md:470`) — приоритет поднимается, если AST-2 упрётся в него.

**Зависимости.** П.3 — миграция 207+ (номера распределяет Astra как координатор после свежей сверки); п.7 — записанное решение до реализации; остальные независимы.

**Критерии завершения.** Каждый пункт закрыт PR-ом с независимым review либо записью отказа с причиной; списочных «сделано» без артефакта нет.
...
Правила портального плана §11 действуют без изменений:
...
Завершение продолжения означает: AST-1 закрыт полностью с квитанцией KB-6; AST-2 прогнан или каждый недоступный пункт назван с внешней причиной; модуль 2 и фото-долг AST-3 закрыты; пакет AST-4 в репо с точным списком владельческих остатков; каждый пункт AST-5 закрыт или имеет записанный отказ. Итог — краткая ведомость продолжения по образцу портальной: блок → результат → SHA/релиз → оставшееся ограничение, по фактам, без процентов «на глаз».
...
Передаваемый результат — фактически заполненная и проверенная база знаний в CRM, доказанные живыми прогонами портальные флоу и готовый к владельческим шагам стор-пакет, а не описание или зелёный CI.
