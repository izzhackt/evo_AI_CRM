...
Текущая таблица пересверена 21 сентября (Asia/Dubai) на main `3ddb6f41`.
...
| CRM-02: права, продавец, дата/месяц, поиск/финансы/UX | CRM-02a: scoped Auth/UI/RPC проверка на изолированной локальной базе 001–208 пройдена; независимое review и CI пройдены. Это локальное QA, не managed/customer acceptance. Остальная часть CRM-02 открыта | MERGED #935, `f5198c37`; managed 208 и production не изменены этим блоком |
...
| CRM-05: операционная воронка | Подтверждённый handoff отделён от наличия дела; local RPC/UI и filters/search/keyboard на 320/390px и desktop проверены; [квитанция](qa/crm-05-operational-handoff-2026-09-20.md) | MERGED #945 `285e784e`; оставшаяся мобильная композиция и дефект сброса owner-filter открыты |
| CRM-04/07: inbox и сообщения | Inbox read-state UX передан root; Student messages остаются у A. Реализация впереди | — |
| Сохранение отдельных блоков карточки / пункт 30 | PR #948: source correction custom staff authority и локальная проверка в работе; положительные QA-сохранения ещё не выполнены | OPEN; локальная 213 — непринятый кандидат, не managed/production delivery |
...
| §11: общие программы, требования, версии, пакеты и review | Устойчивые intake IDs и техническая публикация проверены через actual local Admin/Student UI/RPC. Selection/preparation в открытом #946 пока source-reviewed; requirements/upload/submit/packages/review остаются | MERGED #944 `1e03c6be`; #946/214 ещё не применён/не принят. Подготовка сразу после выбора, только CN/MY/AE/TR/IT/CZ |
