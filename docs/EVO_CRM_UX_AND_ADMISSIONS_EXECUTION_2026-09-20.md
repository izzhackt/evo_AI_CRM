...
| CRM-02: права, продавец, дата/месяц, поиск/финансы/UX | CRM-02a реализован, технические проверки ниже; живой write-путь ждёт отдельной authority. Остальная часть CRM-02 открыта | DRAFT #935; production не изменён |
...
Не подтверждено и блокирует приёмку/merge этого среза: новый разрешённый RPC
...
в реальной форме. UI формы и успеха ещё не осмотрен на фактически созданной
QA-продаже. Миграция 208 не применена в managed Supabase; server release не делали.
...
Разрешённая конкурентная бизнес-проверка остаётся в scoped QA; имитация акторов
не выполнялась. Файлы окончательной проверки имеют суффикс `-final.txt`.
