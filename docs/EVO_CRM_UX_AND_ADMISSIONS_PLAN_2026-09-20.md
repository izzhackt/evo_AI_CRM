...
4. Уточнить только два продуктовых пункта раздела 14 перед зависящими от них изменениями; остальные блоки не требуют повторного согласования.
...
## 14. Два уточнения, которые нельзя выдавать за ответ владельца

1. **Страница «Студенты».** Исходный выбранный пункт обрывается на «Большая сводка стоит». Перенос списка/поиска выше сводки пока не является обязательной задачей. Без уточнения выполнять только выбранные изменения карточки и прямые зависимости общего поступления.
2. **Подтверждение выбора программы.** Предлагается сразу открывать подготовку и показывать её сотруднику, без отдельного одобрения самого выбора. Этот вариант ещё не получил отдельного ответа. До зависимого изменения подтвердить только этот выбор; не открывать заново вопрос доступа — сопровождение уже подтверждено.

Дополнительно для точечного исключения служебной продажи потребуется установить её ID и назначение. Это задача проверки данных, не повод выбирать запись по похожему имени или блокировать остальной план.
