...
- «Задачи»: вкладки «Мои / Поставил я / Вся команда» с числами, полоса
  «Без срока · 3», строки 53 px, отметка выполнения прямо в строке, строка
  «Новая задача…»;
...
схеме 242: он не вызывает чтение чисел, которое меняет 242.
...
- панель лида — рядом с доской (400 px) на компьютере, на весь экран на
  телефоне;
- сплошной красный — не больше одного на странице: по одному на «Воронке
  продаж» («Добавить лида») и в «Отчёте продаж» («Добавить продажу»), на
  «Воронке поступления» — ни одного;
...
Известные проблемы, найденные тем же аудитом; их закрывают отдельные PR, не
этот выпуск:
- доступ (PR с миграцией 243): сервер отказывает приглашённым сотрудникам
  ...
  не может подготовить кабинет из лида; чтение запросов на удаление аккаунта
  при прямом вызове пускает любого приглашённого сотрудника, а должно — только
  Admin (запросов сейчас нет);
...
не менялись. Главные пробелы перед реальной работой — открытые пункты, ждут
решений владельца; этот раздел ни один из них не закрывает.
...
- У учётной записи Admin нет второго фактора (MFA).
