...
creating another document or visa model. It uses two explicit link relations
(`document slot -> university application` and `document slot -> visa case`),
each protected by organization-and-case-qualified foreign keys. One
authenticated Admin/Admissions command replaces the complete bounded link set
for an active slot, requires the slot's optimistic version plus a unique
request id and reason, advances that same slot version exactly once, and writes
the canonical audit result. The staff document-workspace projection returns
sorted linked identifiers through `src/lib/v3/*`; the existing V3 Documents
tab is the sole editor. Direct table writes, polymorphic target ids, a second
status dictionary, student-portal mutation and provider/deployment changes are
outside this slice.
