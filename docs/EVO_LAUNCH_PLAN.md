...
## 2026-09-24 — UX quick wins CRM (критика Impeccable) и роль Sales Manager для Admin

Критика Impeccable staff CRM 24.09 (два независимых агента: дизайн-ревью и
детектор/измерения на живом CRM, только чтение) — 20/40 по Нильсену, пять P1.
Владелец выбрал быстрые победы, объём топ-3, без ограничений. Выпущены
#1047 (подтверждение «Убрать из воронки» с возвратом, danger-зона «Опасные
действия» у сотрудника, сохранённые роли вместо пустой строки), #1046
(«Воронка продаж»/«Воронка поступления», «WhatsApp», H1 = пункт меню, единый
заголовок вкладки «Раздел — EVO CRM») и #1048 (красный только для главного
действия, единый стиль выбора `.v3-choice`, нейтральный бейдж; блокер review —
выбор только цветом в шаблонах форм — исправлен до merge). Каждый PR прошёл
независимое review; #1046 после ребейза подтверждён совпадением git patch-id
и совместным прогоном тестов, #1048 — delta-review: файлы без конфликтов
сверены побайтно, разрешение конфликтов — через range-diff.

App `79f59242279267cae297bde8ffc56bd83b4fbcbd` принят: CI 35932787015 и release
35932812580 SUCCESS, `v3-r35932812580-a1-79f59242`, acceptance record sha256
`f957a7f4afc0f8f16950f6ef127f419ab2c7efd7262d97bfc3a53d8d6c7a966a`, healthy/
restart0, pending отсутствует, arm false с 23:22 UTC 23.09. Честно о попытках:
release 35932057854 attempt 1 остановился на передаче (SSH-таймаут GitHub-
раннера к серверу; на сервере ничего не создано, прод не менялся), attempt 2
(rerun только deploy-джоба) закономерно не прошёл проверку закрытого
артефакта — корректный путь — новый CI dispatch, который и принят. Миграций
нет, ledger 001–240.

Проверка после релиза в живом CRM (Admin, только чтение): пункты меню и
заголовки/вкладки разделов по новым названиям; в меню карточки поступления
пункты 44 px, «Убрать из воронки» в danger-цвете открывает подтверждение
«…Дело останется в «Студентах».» с фокусом на «Отмена», Escape отменяет и
возвращает фокус, карточки на месте (ничего не убрано); выбранные разделы и
фильтры — accent-weak + accent-text + 600; бейдж уведомлений нейтральный.
Сохранение ролей после релиза вживую не повторялось.

Роль Sales Manager назначена сотруднику с ролью Admin 23.09 21:24 UTC через
Настройки CRM по решению владельца (область «Вся организация»); read-only
проверка: гейт записи продаж теперь проходят двое из пяти активных
сотрудников, включая Admin.


## 2026-09-24 — «Студенты»: фасеты и таблица, одна страница; шрифтовые роли CRM

Решение владельца 24.09: страницы «Студенты» и «Сводка по направлениям»
объединены в одну, без карточек-коробок, структура «Фасеты и таблица» (выбор
на странице решений Impeccable, seed 34af73ee); решение §14.1 от 20.09
заменено (записано в PLAN_CHANGES). Шрифты: Golos Text и JetBrains Mono
остаются, построена система ролей. Выпущены #1050 (слева фасеты
«Направление», «Требует внимания», «Статус», «Куратор» с числами из
существующих чтений, клик фильтрует; справа таблица «Студент / Этап /
Следующий шаг / Срок / Куратор / Проблемы» на волосяных линиях; пункт меню
сводки убран, старый адрес `?section=summary#admissions-summary` открывает
ту же страницу; EVO Docs и замещение куратора сохранены; на телефоне фасеты
в кнопке «Фильтры») и #1051 (роли `t-page-title`…`t-figure` в `v3.css`,
один размер заголовка экрана 24 px, ничего меньше 12 px, без uppercase-
подписей, цифры — Golos с табличными цифрами, JetBrains Mono — для
идентификаторов и дат; контрактный тест покрывает весь staff CRM, портал
студента не менялся). Каждый PR прошёл независимое review, для #1050 —
отдельное finish-review по скриншотам (вердикт fix, исправлено до merge);
блокеры review (зона нажатия имени 40 px; сжатие графика на Главной)
исправлены до merge. #1051 после ребейза на #1050 подтверждён patch-id
(range-diff — в независимом delta-review), исключения для файлов «Студентов»
удалены. Новых серверных чтений/RPC, SQL- и RLS-изменений нет.

App `a0f647d93e2036a083f830ee0d34db7142ddd6d1` принят: CI 35944887169 и
release 35944918625 SUCCESS, `v3-r35944918625-a1-a0f647d9`, acceptance record
sha256 `055530f33b82f85460150245de6d24e3a715dda9ca9a13054374f1dbf01f6aab`,
healthy/restart0, pending отсутствует, arm false с 01:59 UTC 24.09. Честно о
попытках: release 35944671775 (CI 35944646956) остановился на secretless
deploy admission — `main_ref_http http_status=403` от GitHub API до любой
передачи на сервер; прод остался на `79f59242` (проверено), arm снят;
принят новый CI dispatch на тот же main. Миграций нет, ledger 001–240.

Проверка после релиза в живом CRM (Admin, только чтение, переходы по
ссылкам без отправки форм): 1440 px — колонка фасетов 252 px, один h1
24/32/600 Golos, заголовки групп 14/600, числа Golos tabular-nums, шапка
таблицы 12/16/500, имя 14/20/600 с зоной нажатия 44 px, нет текста < 12 px,
нет uppercase, нет рамок-коробок, нет горизонтальной прокрутки; числа
фасетов совпадают с read-only SQL (2 активных дела без направления, оба с
куратором); фильтр по куратору сужает таблицу и «Все» до 1, появляется
«Замещение куратора» (форма не открывалась); старый адрес сводки открывает
«Студенты»; EVO Docs — три действия строки ссылками без рамок; 375 px —
«Фильтры» 44 px, строки стопкой, без прокрутки вбок; Главная 1366 px —
h1 24 px, KPI 28 px Golos tabular, график 563 px без прокрутки, подписи
осей ~14 px. Встроенный браузер был скрыт, поэтому потоковый контент
раскрывался вручную в DOM для измерений; замещение куратора и сохранение
форм вживую не выполнялись.


## 2026-09-25 — выпуск r134 («Задачи», очередь дел 241) и план редизайна CRM

App `e4dfd6bae1b779b0a6addeed59d200b66700c1b4` (#1055 «Задачи» — одна очередь
по срокам; #1054 следующий шаг дела и очередь студентов: SQL 241 и серверные
чтения) принят в production 24.09: CI 36069291152 и release 36069325597
SUCCESS, `v3-r36069325597-a1-e4dfd6ba` (`r134.1-e4dfd6ba`), артефакт
10837588051 (`sha256:2f5393de54dbe6ee2c65914c3e474f51345c39eeed7c9a2eecc20b7f37e206c5`).
Шаги конвейера — чтение ledger, deploy pending, «Authenticated read-only case
and Student portal smoke», guards и accept — успешны, accept в 22:52 UTC.
Перед выпуском миграция 241 применена управляемым путём: `evo-schema-ledger`
apply, run 36069140537 (dry-run показал только 241; ledger после применения
001–241), 22:45 UTC. С этого момента студент не видит «Следующий шаг» дела в
iPhone; 25.09 владелец решил оставить так.

Квитанция записана 25.09 по логам GitHub Actions. Из облачной сессии не
прочитаны: запись acceptance на сервере и её sha256, значение
`EVO_PRODUCTION_RELEASE_ARMED` после выпуска и живой UI — сеть сессии не
пускает к `crm.evoadmissions.com`.

Текущий план CRM — [план редизайна 25.09](EVO_CRM_REDESIGN_PLAN_2026-09-25.md),
принят владельцем с поправками разбора. Следующий выпуск — доски (#1053);
дополнение 26.09: доски выпущены 26.09 в `d89eebea` (r135), квитанция ниже.


## 2026-09-26 — дополнение квитанции r134 (`e4dfd6ba`)

Раздел 25.09 выше не переписывается (к нему добавлена только пометка о
выпуске досок); здесь то, что облачная сессия прочитать не могла. Release
36069325597 (CI 36069291152), `v3-r36069325597-a1-e4dfd6ba`: запись acceptance
`v3-r36069325597-a1-e4dfd6ba/v3-acceptance-record.json` создана на сервере в
22:52 UTC 24.09, sha256
`82bb8d87d5521ebf6ff6ea440fbc5d1e88f71399c60fdb86000a7d3fbefad3ce`
(`sha256sum` на сервере 26.09, только чтение). Ревизия, CI, артефакт
10837588051 и его digest в записи совпадают с логами Actions.

Миграция 241: check 36069077394 в 22:44 UTC (в проде не хватало только 241),
apply 36069140537 в 22:45 UTC (dry-run — только 241, ledger после применения
001–241).

`EVO_PRODUCTION_RELEASE_ARMED` вернула в false координирующая сессия в 22:52
UTC 24.09 — по её записи. Переменная хранит только текущее значение, поэтому
значение тех минут повторно не читается.

Живая проверка координирующей сессией 25.09 (Admin, только чтение):
- «Задачи»: вкладки «Мои / Поставил я / Вся команда» с числами (аудит 26.09
  уточняет: числа «Поставил я» и «Вся команда» появляются только после
  открытия этих вкладок), полоса «Без срока · 3», строки 53 px, отметка
  выполнения прямо в строке, строка «Новая задача…»;
- «Календарь»: «Сводки на Главной» нет, «Задача по студенту» нейтральная,
  сетка сохранена, высота документа 1227 → 1117 px;
- «Студенты» (ещё прежняя страница) открываются: 2 строки, без
  предупреждений.


## 2026-09-26 — выпуск r135 (`d89eebea`): доски, «Студенты», «Сообщения»; миграция 242

App `d89eebea6d739317d308f832b873a9a5c30b258e` принят в production 26.09:
CI 36240181315 и release 36240200125 SUCCESS, `v3-r36240200125-a1-d89eebea`
(`r135.1-d89eebea`), артефакт 10905408779
(`sha256:b6e789a658539f845cdfb36b3733764af1ae2939d7b12a9cd3bfa57317a5246d`),
образ `sha256:905bac6111a3303bc1af7b50248c36561569bcfbb44b458df5545200edeb5e65`,
accept в 11:58 UTC. Acceptance record sha256
`afbea0c425903118b7631f63ec75aa51007ca3230c7dadc4956bfcd6dc435a2a`,
`current-v3-accepted.json` указывает на этот выпуск; `evo-crm-app-1`
healthy/restart0, запущен в 11:57:33 UTC; pending отсутствует; arm false с
11:58 UTC 26.09; `/api/health` 200 на crm и app.

Шаги конвейера успешны: secretless admission, чтение ledger, guards GitHub и
Supabase, deploy pending, «Authenticated read-only case and Student portal
smoke» — 11 контрольных точек (`admin_login`, `case_route`, `case_contract`,
`admissions_pipeline`, `students_queue`, `evo_docs`, `team_chat`,
`student_login`, `student_overview`, `student_documents`,
`student_navigation`), accept; шаги отката пропущены.

Состав относительно r134 (`e4dfd6ba`): #1053 — доски на всю ширину; #1057 —
план редизайна и квитанция r134 (документы); #1058 — порядок сообщений в
«Сообщениях»; #1059 — «Студенты»: очередь дел, «Ожидает начала», «Принять
дело» в панели, контрольные точки smoke `students_queue` и `evo_docs`.

Миграция 242 применена 25.09 облачной сессией управляемым путём до выпуска,
который отгружает её код: check 36172115868 (в проде не хватало только 242),
apply 36172178566 в 18:15 UTC (dry-run — только
`242_platform_case_queue_pending_view.sql`; ledger после применения — 242
версии, равен репозиторию). С 18:15 UTC 25.09 до выпуска работал r134 на
схеме 242. 242 меняет оба чтения очереди дел (строки и числа) и их общее
условие; r134 не вызывает ни одно из них — из модуля очереди он вызывает
только сохранение следующего шага, которое 242 не меняет.

Сверено 26.09 только чтением: прогоны, шаги и логи — `gh run view`; запись
acceptance, указатель, отсутствие pending и контейнер — на сервере
(`sha256sum`, `cat`, `docker inspect`); arm — GitHub API; health — HTTP GET.

Живая проверка аудита 26.09 (Admin, только чтение, 375, 1280 и 1440 px,
формы не отправлялись):
- доски на всю ширину, горизонтальной прокрутки страницы нет: «Воронка
  продаж» — 6 колонок при 1280 px; «Воронка поступления» — 5 колонок, около
  260 px при 1440 и 202 px при 1280;
- рельс иконок 64 px от 768 до 1535 px, полное меню 260 px при 1600 px;
- панель лида — рядом с доской (400 px) на компьютере, на весь экран на
  телефоне;
- сплошной красный на досках и в отчёте продаж: по одному на «Воронке
  продаж» («Добавить лида») и в «Отчёте продаж» («Добавить продажу»), на
  «Воронке поступления» — ни одного. Правило «один красный на странице»
  выполнено не везде: в Lead 360 и во вкладке «Обзор» Student 360 — по 7
  сплошных красных кнопок (объём Э4), в «Командном чате» — 2 (красная
  буквенная плитка канала; инициалы — общий блок Э1);
- «Студенты»: числа вкладок очереди совпадают с базой (в работе 2, мои 1,
  «Ожидает начала» 0, без куратора 0, закрытые 0); «Быстрый просмотр» с
  редактором следующего шага; вкладка «Ожидает начала» есть;
- EVO Docs: «На проверку 0 · Исправить 0 · Все 2» совпадает с базой;
- меню карточки на доске поступления открывается в верхнем слое и
  закрывается Escape.

Известные проблемы, найденные тем же аудитом; их закроют отдельные PR (на
момент записи 26.09 ещё не открыты), не этот выпуск:
- доступ (PR с миграцией 243): сервер отказывает приглашённым сотрудникам
  поступления в перемещении карточки на доске, хотя интерфейс его предлагает;
  назначение куратора не работает ни у кого, включая Admin, — функция
  вызывает удалённую вспомогательную проверку; сохранение требований к
  документам отказывает приглашённым сотрудникам поступления; Sales Manager
  не может подготовить кабинет из лида; одно чтение, предназначенное только
  Admin, при прямом вызове проверяет роль шире, чем нужно (подробности — в
  отчёте аудита, не в публичном репозитории);
- честное состояние (PR исправлений): ложная красная ошибка базовых
  чек-листов на вкладке «Документы» дела; «Без действия» на доске продаж
  считает уже переданного лида; ложная тревога «База данных» в «Настройках».


## 2026-09-26 — аудит готовности 26.09

Read-only аудит production после выпуска r135: план и GitHub, доступ по
ролям, эксплуатация, живой UX продаж и поступления. Код, данные и настройки
не менялись; сам аудит отметил два побочных действия на сервере — одну
пустую команду `docker exec`, ничего не изменившую, и временный файл без
секретов в `/tmp`, который удалён, отсутствие проверено. Главные пробелы
перед реальной работой — открытые пункты, ждут решений владельца; этот
раздел ни один из них не закрывает.

- WhatsApp не подключён к платформе: приём WAHA в приложении выключен
  (проверено 26.09), таблицы переписок и состояния сессии пусты. Сессия
  шлёт события на прежний, уже удалённый адрес вебхука — около 30 тыс.
  отказов 403 в сутки (проверено 26.09 по журналу WAHA за 24 ч). Решение
  владельца: снять прежний вебхук либо отдельно разрешить подключение
  (действие у провайдера).
- Путь заявки с сайта в CRM ни разу не пройден целиком: квитанций приёма с
  сайта 0 за всё время. Решение: одна помеченная тестовая заявка владельца с
  архивированием после проверки — агентам создавать заявки нельзя.
- Уведомлений вне приложения нет: ни письма о новой заявке или назначенной
  задаче, ни расписания для обработчика просрочек.
- Нет свежей резервной копии Storage: ежедневные копии Supabase файлы не
  включают, последний отдельный экспорт базы вместе со Storage — 08.09.
- Защита входа учётной записи Admin не усилена (подробности — в отчёте
  аудита, не в публичном репозитории).
- Сервер: вход по SSH и сетевой фильтр не ужесточены (проверено 26.09;
  подробности — в отчёте аудита, не в публичном репозитории). Нужно окно
  работ, согласованное с владельцем.
- Нет DMARC у домена, с которого идут письма подтверждения, приглашения и
  восстановления (проверено 26.09).
- Допуск выпуска изредка падает на `main_ref_http 403` (например, release
  35944671775 24.09): оба шага secretless admission читают `git/ref/heads/main`
  без авторизации. Вероятная причина — анонимный лимит GitHub API на адрес,
  общий для раннеров; прямого доказательства нет. Исправление —
  авторизованный запрос или `git ls-remote` с повтором, отдельным PR с
  review, так как это контракт выпуска.

Источник — аудит 26.09 (read-only SQL, SSH и `gh`); пункты с пометкой
«проверено 26.09» пересняты только чтением при записи этой квитанции.
