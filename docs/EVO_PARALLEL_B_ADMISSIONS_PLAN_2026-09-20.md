...
Статусы пересверены 21 сентября (Asia/Dubai) на main `3ddb6f41`; открытый #946
учитывается отдельно от merged исходников. Пункт 26 в выбранном техническом scope
уже выполнен через #937/#939; массовая контентная волна сюда не добавляется.
...
`93466099`; 214 ещё не применена, положительный Student-путь не выполнен.
Локальный порядок зависит от корректировки непринятой 213 в #948 и отдельного
QA-сценария. Requirements/upload/submit/packages/review и полный web/iPhone UI
остаются следующими, ещё не реализованными блоками.
