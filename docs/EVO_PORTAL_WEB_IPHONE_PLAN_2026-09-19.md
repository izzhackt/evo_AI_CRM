...
Название приложения: **EVO admissions**. Используй оригинальный логотип EVO, включая корректную адаптацию существующего знака для иконки приложения. Веб-портал остаётся на **https://app.evoadmissions.com**; новый домен или отдельная копия продукта не нужны. Сотрудники продолжают работать на **https://crm.evoadmissions.com**.
...
| Имя и адрес | EVO admissions, оригинальный логотип; веб-портал — app.evoadmissions.com |
...
### Соседняя сессия OTHER: конкретные границы

Владелец подтвердил, что другая сессия Claude Code уже выполняет `EVO_OTHER_FABLE_PLAN_2026-09-19.md` и получила указание закончить все slices с последовательным merge. Portal/App исключён из её задачи; этот план не отменяет и не перехватывает её работу. Отдельные сессии и worktree изолируют рабочие файлы, но сами по себе не разделяют main, Supabase и production.

| Область | Кто ведёт | Как взаимодействуют работы |
| --- | --- | --- |
| Staff-экраны Other, воронка куратора, staff-задачи | OTHER | Portal не переписывает эти экраны и не повторяет OTH-1/2 |
| Новые общие контракты задач, финансов, выбора вузов и case-chat из OTH | OTHER до интеграции соответствующего блока | Portal читает согласованные контракты, добавляет свою сторону после появления реальной зависимости; уточнения вносит с одним владельцем общего файла |
| Портальные веб-экраны и их стили | Portal/Web | Изменения изолированы от staff-оболочки; общие primitives меняются с проверкой обеих сторон |
| iPhone и его сборка | Portal/iPhone | Общие данные и backend, отдельный клиент; не отдельная БД или staff-приложение |
| Анкета, approval, общие права | Portal с учётом действующей Auth/email-работы | Не перезаписывать новые staff permissions, resource-scoped ограничения и уже внесённые исправления |
| Миграции и применение схемы | Один согласованный координатор в каждый момент | Пока OTHER ведёт свою цепочку, Portal не занимает его номера и не запускает параллельный apply |
| Production release и arm/disarm | Одна сессия-владелец конкретного релиза | Вторая сессия не меняет arm, не запускает competing deploy и не выключает разрешение чужого активного релиза |

До изменений общего файла сверяй актуальные PR и передавай изменение его текущему владельцу либо дождись merge этой зависимости. Согласование фиксируется в репозиторном плане/PR с указанием ветки, файла или контракта; при отсутствии доступного канала связи не считай молчание передачей владения. Продолжай независимые задачи. После merge зависимости обнови свою базу и адаптируй вызовы к принятой версии, не восстанавливай устаревшие контракты из старого worktree.

На проверке 19.09.2026: PR [#861](https://github.com/izzhackt/evo_AI_CRM/pull/861), head `324103e5b38d4beae2d948d9e60be99711c33783`, открыт с `186_platform_admissions_pipeline_board.sql`; PR [#862](https://github.com/izzhackt/evo_AI_CRM/pull/862), head `6e1d3a7cc946bf9fa77688ed6eaeaaa0ff770fe2`, открыт с `189_platform_application_author_and_optional_program.sql`. При этом в main `c033ec466a73e39f0be591c182ec41c377464279` уже есть **`186_platform_student_invite_conflict_codes.sql`**. Это реальное пересечение номеров, а не свободный слот для OTH. Координатор OTHER должен перед merge пересчитать ещё не применённую цепочку с учётом текущего ledger и зависимостей. Portal не исправляет чужие PR молча и не применяет второй файл под номером 186. Снимок статуса требуется обновить перед действием; он не утверждает, что конфликт сохранится к старту исполнения.

В текущем `.github/workflows/evo-fast-release.yml` уже есть общая `concurrency.group: evo-production-release` с `cancel-in-progress: false`. Используй этот workflow, но учитывай, что его очередь сама по себе не защищает внешние действия двух сессий с arm/disarm и ручным schema apply. Владельца этих действий нужно установить до их начала. Выпуск использует свежий согласованный main и не должен терять уже слитые изменения другой работы.
