...
Статус на 22 сентября 2026: source review и CI приняты на `c1fcda9b`;
один ordinary Admin UI-прогон 390/320/1440 и strict closure приняты независимым
review `669af3dc`. ROOT подтвердил передачу QA (`d21f93bc`); ресурсы A закрыты.
Доказательства — в [source-квитанции](qa/team-chat-channel-time-ui-source-2026-09-22.md)
и [actual-квитанции](qa/team-chat-channel-time-ui-actual-2026-09-22.md).
Main `ac165cf9` интегрирован без изменения четырёх A code/test-файлов;
финальные integration review/CI и merge #1030 остаются следующим шагом.
...
Новое подтверждение визуального направления не требуется. Craft-floor прочитан
непосредственно перед UI-правкой после независимого принятия этого плана.
...
До реализации исторические снимки показывали короткие Общий/Продажи;
план допускал ellipsis названия рядом с временем, сохраняя полный текст в DOM
и доступном имени ссылки. Фактический прогон уточнил результат: на390/320
названия видны полностью, в desktop rail288px «Продажи» сокращается до «Прода…».
Это принятый предел компактной строки; полное визуальное имя на desktop не
заявляется. Пустое Поступление time не получает. Перенос времени или расширение
rail в этот срез не добавлены.
...
Ниже — порядок, зафиксированный до реализации. Его выполненную часть,
исходный zero-seen STOP и отдельное разрешение finite51 фиксирует actual-квитанция;
она не разрешает повторное использование уже переданного ROOT окна.
