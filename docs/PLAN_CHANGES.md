...
## 2026-08-05 - Add P2R4 Local Migration Simple-Query Validation Repair

Block-ID: `EVO-P2R4-LOCAL-MIGRATION-SIMPLE-QUERY-PLAN-2026-08-05`

Source: merged PR #120 at
`68fe755e9c7e6bbd92218ad6566aff3b7431b6f6`, the actual clean local Supabase
failure/reproduction on pinned CLI 2.110.0 and diagnostic CLI 2.109.1, the
successful exact-project simple-query reconstruction, PostgreSQL `ALTER TYPE`
documentation, and the official Supabase migration-list/repair contract:

- <https://www.postgresql.org/docs/15/sql-altertype.html>
- <https://supabase.com/docs/reference/cli/supabase-start#supabase-migration-list>
- <https://supabase.com/docs/reference/cli/supabase-start#supabase-migration-repair>

Change type: local validation implementation and acceptance-procedure repair
for immutable merged migration history. This is not a schema, API, product,
provider, target-architecture or production-authority change.

Affected plan section: `/goal-evo-platform-p2r4-local-migration-simple-query-plan`,
the reusable P2 local validation gate and the current implementation merge
order.

Evidence:

- PR #120 merged BW8A as current `origin/main` and owns immutable migration
  `059_platform_student_document_intelligence.sql`.
- Actual `npm run test:supabase:local` with pinned Supabase CLI 2.110.0 fails
  while applying migration 059 with PostgreSQL SQLSTATE `55P04`,
  `unsafe use of new value "document_validation"`. A diagnostic run with CLI
  2.109.1 fails at the same statement and SQLSTATE, so downgrading one CLI
  release is not evidence or a repair.
- Migration 059 begins with six `ALTER TYPE ... ADD VALUE` statements and later
  uses those values in the same file. PostgreSQL documents that an enum value
  added inside a transaction block cannot be used until that transaction has
  committed. The failing CLI migration execution does not provide that commit
  boundary before the later same-file use.
- A clean exact-project local Supabase stack started with
  `SUPABASE_DB_MIGRATIONS_ENABLED=false`. Applying canonical migrations 001-059
  sequentially inside the database container through
  `psql -X -v ON_ERROR_STOP=1` simple-query execution passed every file,
  including migration 059, without changing any migration bytes.
- Only after all SQL succeeded, the official
  `supabase migration repair --local --status applied <version>` command marked
  each exact version 001-059 in the local ledger, and
  `supabase migration list --local` matched all 59 canonical versions exactly.
  PostgREST then loaded 112 relations and 164 RPCs; Database, PostgREST, Auth,
  Storage and Kong were healthy.
- Exact-project containers, volumes, network and lock were removed after the
  proof. The separate EVO Inbox stack and data were unchanged.
- P4B remains an uncommitted/stashed candidate on migration 060. It is not
  shared source of truth and remains paused while P2R4 repairs the prerequisite
  validation gate.

Decision:

- Make this docs-only P2R4 amendment the sole active block. It consumes no
  migration number and preserves merged migration 059 byte-for-byte. P4B and
  later dependent implementation remain paused until P2R4 is independently
  reviewed, controller-merged and green on exact-main CI.
- After this amendment merges, authorize one separate implementation PR to
  change only the local validation harness and its focused negative tests:
  `scripts/test-supabase-local-reset.sh` and
  `tests/supabase-local-reset-harness.test.mjs`. No migration, application,
  provider, deployment or Inbox file enters this block.
- The clean local gate must refuse linked project refs, acquire the existing
  exact-project singleton lock, remove only stale resources carrying the exact
  `evo-platform-local` label, and start one clean stack with database migration
  auto-execution disabled. The local pre-schema bootstrap may use the CLI's
  bounded `start --ignore-health-check`, because PostgREST cannot load the
  `platform` schema before canonical SQL creates it; this exception must never
  hide database-container startup failure or escape this exact local project.
- It must first require fail-closed database-container readiness, then apply
  every canonical contiguous migration file sequentially inside that container
  with `psql -X -v ON_ERROR_STOP=1` simple-query execution. Any SQL failure
  stops immediately and forbids ledger repair.
- Only after every SQL file succeeds may the harness use the official local CLI
  `migration repair --status applied` command for the exact canonical versions.
  It must then use official `migration list --local` and fail unless filesystem
  and local-ledger versions match exactly in count, order and value.
- Only after SQL and ledger equality succeed may it require fail-closed
  Database, PostgREST, Auth, Storage and Kong readiness and run the real
  Auth/PostgREST/RLS/browser, private Storage and PGMQ gates. After browser
  mutation it must repeat the same clean rebuild, full readiness and exact
  ledger comparison rather than trusting the first run.
- Cleanup remains exact-project only and must prove no owned container, volume,
  network or singleton lock remains while Inbox resources and state are
  unchanged. Broad prune, Docker daemon restart and unrelated mutation remain
  forbidden.
- Focused negative harness tests must prove that SQL failure cannot forge an
  applied ledger, repair/list mismatch fails, linked refs are rejected, and
  cleanup never broadens beyond the exact disposable project.
- This harness is local validation infrastructure only. It must never become a
  linked, managed, staging or production deployment runner. A managed
  staging/branch run through the official Supabase migration path remains an
  external release proof and blocker before any production claim.

Validation impact:

- This docs-only amendment updates only the launch/long-run contracts, this
  append-only decision log and current status. ADR 0014-0016, canonical TZ,
  `CONTEXT.md`, schema/API/product/provider contracts and migration files remain
  unchanged.
- The later P2R4 implementation must prove both clean rebuilds through 001-059,
  exact ledger equality, all existing real local service gates, the four new
  negative harness cases, exact cleanup, Inbox preservation, focused unit and
  security tests, all four exact-head CI jobs and SHA-bound independent review.
- The managed staging/branch official migration path must be exercised and
  recorded separately before release; local simple-query proof cannot satisfy
  that external gate.
- `real-provider-proof: not-required`. This amendment performs no provider call
  and processes no customer data.

Reviewer notes: P2R4 repairs how immutable canonical SQL is validated locally;
it does not reinterpret, split, edit or replace migration 059. The ledger is
repaired only after independently successful SQL, never used to conceal a
failed or partially applied migration.

## 2026-08-05 - Add P2R5 Durable-Work Pointer Projection Repair

Block-ID: `EVO-P2R5-DURABLE-WORK-POINTER-PROJECTION-PLAN-2026-08-05`

Source: merged P2R4 plan PR #122 at
`6a89a62ca1f258c574e9cf55a00e32697c1bc94b`, the separate two-file P2R4
implementation worktree, two successful clean 001-059 simple-query and local
ledger replays, 28/28 accepted-browser scenarios, the passing private Storage
gate, the failing real PGMQ claim, immutable migration 059 and its durable-work
inventory.

Change type: forward-only compatibility repair plan for a latent runtime defect
introduced by immutable migration 059. This is not a new queue, worker,
provider, UI, deployment or production authorization.

Reason:

- P2R4 removed the CLI transaction blocker without editing migration 059 and
  reached the real PGMQ gate. The generic
  `platform.claim_durable_work(integer,text,uuid)` then raised PostgreSQL
  record-field error `42703`.
- Migration 059 adds durable-work row columns
  `source_document_version_id`, `source_extraction_run_id` and
  `source_profile_export_id`, but its final response projection reads
  nonexistent row fields named `document_version_id`,
  `document_extraction_run_id` and `student_profile_export_id`.
- Migration-045 PGMQ tests execute at their immutable historical boundary, so
  they could not detect a defect introduced by the later migration-059
  replacement function. Skipping or mocking the final P2R4 PGMQ gate would hide
  a real regression and is rejected.

Decision:

- Make this docs-only P2R5 amendment the sole active block. Keep migration 059
  byte-for-byte immutable and pause the unmerged P2R4 implementation.
- Reserve next-free migration 060 for one forward-only replacement of the
  existing generic service-only claim. Preserve its public JSON keys but map
  them to the actual `source_*` row columns.
- Preserve the exact generic request hash, replay identity, PGMQ visibility,
  lease/attempt/event writes, source-shape checks, empty search path and current
  service-only ACL. Do not add BW8B's filtered claim, validation-input resolver,
  enum operation, scanner, second queue or product behavior.
- Require a focused source-contract test to compare the immutable-059 and
  new-060 generic function bodies and fail unless normalizing only the three
  corrected row-field references makes them identical.
- Limit the later implementation PR to migration 060, one focused exact-060 SQL
  regression, the minimal disposable-PostgreSQL harness wiring for that test,
  and focused assertions in the existing document-intelligence domain test.
- Prove one legacy P2G claim and the three typed BW8 source-pointer variants,
  including public-key values/nulls, replay/attempt/event identity,
  unauthorized denial and malformed-source rejection.
- After P2R5 merges, rebase the existing two-file P2R4 implementation onto
  exact main and rerun the complete clean 001-060 path twice through ledger,
  Auth/RLS/browser, private Storage, PGMQ and exact cleanup.
- P2R5 consumes migration 060. P4B's stashed 060 and BW8B's closed-PR 060 are
  stale and not mergeable. Their accepted business contracts remain valid, but
  each implementation must restart from fresh main, choose the then-next free
  number (expected 061), and remove any duplicate ownership of this generic
  compatibility repair.

Validation impact:

- This docs-only amendment changes only `docs/EVO_LAUNCH_PLAN.md`,
  `docs/EVO_PLATFORM_LONG_RUN_PLAN.md`, this append-only decision log and
  `docs/platform/current-status.md`. It changes no migration, runtime, API,
  provider, deployment, Inbox, ADR, TZ or production state.
- The later P2R5 implementation requires immutable-history proof, focused unit
  and exact-boundary SQL regression, full disposable PostgreSQL/security gates,
  a combined P2R4-harness integration proof, scoped secret scan, all four
  exact-head CI jobs and SHA-bound independent review/controller merge.
- P2R4 remains a local validation runner only. Managed staging/branch proof
  still requires the official external Supabase migration path and separate
  authority.
- `real-provider-proof: not-required`. This amendment processes no customer
  data and calls no provider.

Reviewer notes: pending independent exact-head plan review. The runtime failure
is evidence that P2R4 must stay fail-closed, not justification to weaken its
PGMQ acceptance gate.
