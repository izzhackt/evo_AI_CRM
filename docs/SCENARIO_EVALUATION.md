...
Generated: 2026-06-24T07:08:29.662Z
...
- Temp DB: `/var/folders/p4/c09jb8gd4qngjbkr1cqfh8rh0000gp/T/evo-crm-scenarios-91822-1782284908184/edu-admin.db` (isolated copy; removed after the run unless
...
| S04 | Navigation and admissions copy | Staff shell exposes admissions CRM navigation and metadata copy. | Admin dashboard includes EVO CRM shell plus applications/documents/finance navigation labels. | PASS | dashboard shell contains EVO CRM nav links for applications, documents, and finance |
| S05 | Role visibility | Finance role can use finance overview but not admissions document/application queues. | Finance session loads /finance and redirects from /applications and /documents to /dashboard. | PASS | finance 200; applications 307; documents 307 |
...
| S08 | Admissions Pipeline | Add lead Server Action creates an EVO admissions lead. | Submitting the real /sales lead form inserts a lead with source, target country, manager, and amount. | PASS | created lead 7 status new, Canada, 120000 KGS |
| S09 | Admissions Pipeline | Move lead Server Action updates lead status and activity. | Submitting a lead move form changes status to meeting and records a status activity. | PASS | lead 1 moved to meeting; status activities 1 |
| S10 | Admissions Pipeline | Convert lead Server Action creates Student 360 client and marks lead won. | Converting an unconverted lead creates a client, links it to the lead, and sets status won. | PASS | lead 8 converted to client 3 |
...
| S28 | WhatsApp webhook and settings | Settings save integration token, verify endpoint works, and incoming webhook creates lead/conversation. | Admin settings form saves verify token; GET webhook echoes challenge; POST incoming message creates conversation and linked lead. | PASS | verify ok; inbound conversation 4, lead 9 |
| S29 | Calls and telephony | Calls page and telephony webhook enforce key handling and insert calls. | Settings tel key blocks invalid webhook; valid webhook inserts a call and links by lead phone. | PASS | invalid key 403; call 5 linked to lead 1 |
