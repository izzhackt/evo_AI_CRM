...
DECLARE actor UUID; j platform_private.kb_exports; found_options JSONB; count_entries INTEGER;
BEGIN
 ...
 IF p_mode='start' THEN
   ...
   WITH RECURSIVE eligible AS (
     ...
     SELECT n.* FROM eligible n WHERE CASE WHEN jsonb_array_length(coalesce(p_options->'ids','[]'))>0
       ...
       ELSE (NOT(p_options ? 'area') OR n.area=p_options->>'area') END
     ...
     SELECT j.id,row_number() OVER(ORDER BY snapshot->>'id',(snapshot->>'version')::INTEGER)::INTEGER,platform_private.kb_export_snapshot(snapshot) FROM snapshots;
...
RETURNS JSONB LANGUAGE plpgsql VOLATILE SECURITY DEFINER SET search_path='' AS $$
DECLARE j platform_private.kb_exports; result JSONB; b platform_private.kb_blobs;
BEGIN
 ...
 IF p_mode='expire' THEN
   ...
   UPDATE platform_private.kb_exports SET parts='[]',options='{}' WHERE id=j.id;
 ...
 IF p_mode='claim' THEN
   ...
   UPDATE platform_private.kb_exports SET state='running',lease_id=p_lease,lease_until=now()+interval '5 minutes',
     completed_entries=0,error_code=NULL WHERE id=j.id RETURNING * INTO j;
   RETURN to_jsonb(j);
 END IF;
 ...
 ELSIF p_mode='failed' THEN
   UPDATE platform_private.kb_exports SET state='failed',error_code=CASE WHEN p_data->>'code' ~ '^knowledge_[a-z_]+$' THEN p_data->>'code' ELSE 'knowledge_export_failed' END,lease_until=NULL WHERE id=j.id;
...
RETURNS JSONB LANGUAGE SQL STABLE SECURITY DEFINER SET search_path='' AS $$
 SELECT coalesce(jsonb_agg(to_jsonb(t)),'[]') FROM (
   SELECT id,organization_id FROM platform_private.kb_exports WHERE organization_id=p_organization_id
     AND expires_at<=now() AND state<>'expired' ORDER BY expires_at LIMIT 10) t
$$;
