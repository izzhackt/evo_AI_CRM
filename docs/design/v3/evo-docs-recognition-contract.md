...
- Внутреннее чтение истории вызывает ту же STABLE RPC через JSON POST текущей сессии:
  это сохраняет SQL NULL для отсутствующих source/cursor. GET-параметры SDK превращают
  null в текст "null", который корректно отклоняет SQL. Внешний GET, права, курсор
  и запрет создания заданий при чтении остаются прежними.
