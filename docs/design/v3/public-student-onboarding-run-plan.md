...
2. Реальный Supabase email/password signup без письма подтверждения.
   `disable_signup=false`, `mailer_autoconfirm=true`; остальные Auth методы
   и `mailer_allow_unverified_email_sign_ins` не менять. Только живая сессия,
   повторно прочитанная через Auth, может сохранить заявку. Ответ signUp без
   сессии не означает успешную регистрацию; показать ошибку/обычный вход.
