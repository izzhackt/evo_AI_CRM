...
Текущий checkpoint: main `b1ebeddf`, CI34763194546. Передача дела уже проходит
проверку старой и новой сессией куратора; следующий отказ — создание общего
требования к документу даже для System Admin. Forward158 восстанавливает только
этот защищённый путь конфигурации, не расширяет права дел и не меняет155–157.
Причина и порядок проверки записаны в PLAN_CHANGES; release34763572090 пропущен,
arm=false подтверждён. Production остаётся на принятом S1/D1 `77cde9ba`.
