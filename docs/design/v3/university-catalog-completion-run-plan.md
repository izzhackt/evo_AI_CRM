...
Follow-up #729, 2026-09-11 — IN PROGRESS: владелец просит убрать служебные
пометки неопределённости из staff/Student reader. Неизвестные поля и наборы
не выводятся, редакционный блок уточнений убирается; реальные описания, ссылки,
фото и закрытые сроки сохраняются. Данные/публикации не меняются. Пробелы
сообщаются в чате. Проверить реальные карточки с пустыми полями, известными
датами и закрытым набором; общий reader покрывает staff, Student и Admin preview.
После exact-head review/PR/full CI/release проверить тот же сервер через tunnel.
Это уточнение supersedes старое требование показывать unknown-пометки ниже.
