...
11 сентября 2026. Первичное завершение: приложение `892558b2`, 143 карточки.
Текущий принятый app `4dc5ead9`; завершённое продолжение #729 описано ниже.
...
Follow-up #729, 2026-09-11 — COMPLETE: владелец просит убрать служебные
...
фото и закрытые сроки сохраняются. Установленные факты из смешанных заметок
переносятся в существующие описания/подписи; только затронутые карточки проходят
обычную Admin-публикацию. Исходные заметки/источники, null-поля, статусы и identity
сохраняются. Пробелы сообщаются в чате. Проверить карточки с пустыми полями, известными
...
Три исторические языковые программы остаются явно архивными, не текущими наборами.
Подготовлено 82 обновления существующих карточек (110 текстовых полей); все 143
вуза и 251 программа сохранены. Единственные изменения metadata: EMA language
становится null из-за конфликта источников; в двух языковых курсах ZISU сохраняется
«Китайский» без пометки о неизвестном языке пояснений. Исходные notes, ID,
фотографии, источники, даты и статусы не меняются. 24 focused проверки, scoped ESLint
и production build пройдены; Admin-вход через существующий tunnel восстановлен.
PR730 прошёл independent exact-head review и слит; приложение `1a7f870b` принято
на сервере через CI34567818877 / release34568354768, arm=false. Публикация
82 обновлений завершена обычным Admin batch; после перезагрузки 143 актуальны,
0 конфликтов и 0 оставшихся. Реальный staff ECUST/APU/EMA и Admin Student EMA
подтвердили новый общий reader. Найденная в заголовке студенческой страницы старая
фраза вне общего reader исправлена в обоих real/preview wrappers через PR731.
CI34573219849 и release34573853260 приняли `4dc5ead9`; live preview показывает
нейтральный текст, фото и сохранённые условия. Повторная сверка каталога:
143 актуальны /0 конфликтов /0 оставшихся. Данные и миграции не повторялись.
Принятый контейнер healthy/restarts0, pending отсутствует, hash проверен, arm=false.
Доказательства и границы проверки:
[release record](references/2026-09-11-university-clean-reader-release.md).
...
2. GitHub main — общая истина. Первичная ветка `izzhackt/university-catalog-completion`
   ...
   в `892558b2`; очистка reader — PR730/731 до `4dc5ead9`.
   Последующее docs-only закрытие не требует нового выпуска.
   Не трогать другую грязную рабочую копию/ветку Inbox.
3. Последний принятый app `4dc5ead9`, schema001–151, каталог143; перед новой
   работой проверить живые SHA/ledger/arm. Release34573853260 attempt1 принят,
