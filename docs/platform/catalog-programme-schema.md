...
Основание — измерение, а не предположение. Извлекатель отказался брать
длительность у 34 из 104 программ именно потому, что источник даёт два значения:
«3 года при очном обучении и 4 года при заочном», «2 года full-time или 4 года
part-time». Одна запись с одной ячейкой длительности вынуждала бы выбрать одно
из двух молча. Значение `online` добавлено потому, что в источниках встречаются
программы с пометкой Online Learning.
...
CREATE TABLE platform.catalog_programmes (
  ...
  -- Режим обучения входит в состав программы, а не в её описание: очная и
  -- заочная формы имеют разную длительность и являются разными записями.
  study_mode platform.catalog_study_mode NOT NULL,
  -- Пустое значение означает «в источнике дословного факта нет».
...
**Уникальность включает уровень и режим обучения.** У одного заведения бывают
Diploma in Business и Bachelor of Business одновременно; без уровня в ключе они
конфликтовали бы. Режим добавлен по результату измерения: 34 из 104 программ
указывают длительность отдельно для очной и заочной формы. Нормализация имени
повторяет существующую `platform_private.normalize_catalog_institution_name`.

**Колонки языка обучения нет.** Извлекатель прошёл все 104 документа и нашёл
язык преподавания в нуле из них: источник об этом не говорит. Поле, которое
гарантированно пустое, создаёт место для догадки, поэтому в первой версии его
нет. Оно добавляется, когда появится источник.
...
11. Программа с одним именем и уровнем сосуществует в очной и заочной форме
    как две записи; попытка создать две записи с одинаковыми именем, уровнем и
    режимом отклоняется.
12. Уровень программы не выводится из заголовка документа или таблицы.
...
| 1C | Извлекатель хранилище → кандидаты | выполнено, PR #317 |
