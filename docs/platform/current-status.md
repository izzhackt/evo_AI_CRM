...
- Status: Verified snapshot (проверенный снимок) — требует обновления после
  слияния изменений в `main` или развёртывания на сервере
- Last verified: 2026-07-12
- Sources: базовый коммит до документирования `12107b89`, текущие ещё не
  закоммиченные документы, `git`, GitHub CLI, файлы репозитория, DNS-проверка
  через `dig`, публичные HTTPS-запросы
...
| Рабочая ветка до документирования | Технический ответственный | Базовое состояние: `izzhackt/evo-source-of-truth`, коммит `12107b89`; до этого изменения документации были на 33 коммита и 558 файлов впереди `origin/main` | 2026-07-12 | `git rev-list`, `git diff --shortstat` |
| Доставка изменений в `main` | Технический ответственный | Открытого PR в `main` нет; поэтому `main` пока не содержит весь актуальный контекст платформы | 2026-07-12 | `gh pr list --state open` |
...
| Командная база знаний | Ответственные за бизнес и операционную работу | В текущих ещё не закоммиченных файлах созданы центральный индекс, обзор платформы, матрица владельцев данных и onboarding; бизнес-факты требуют владельцев и регулярной проверки | 2026-07-12 | `docs/README.md`, `docs/business/`, `docs/team/` |
...
1. Провести review рабочей ветки, открыть PR и после зелёных проверок обновить
   `main`.
