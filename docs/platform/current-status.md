...
- Status: Verified snapshot (проверенный снимок) — требует обновления после
  слияния PR #32, доставки интеграционной ветки в `main` или развёртывания на
  сервере
- Last verified: 2026-07-12
- Sources: интеграционный базовый коммит `12107b89`, основной коммит
  документации `86c4175d`, исправление фактов `6f4b3391` и PR #32; актуальный
  head PR проверяется через `gh pr view 32`; также использованы `git`, файлы
  репозитория, DNS-проверка через `dig` и публичные HTTPS-запросы
...
| Интеграционная основа | Технический ответственный | Ветка `izzhacktcodex/waha-integration`, коммит `12107b89`; она на 33 коммита впереди `origin/main` | 2026-07-12 | `git rev-list`, `git rev-parse` |
| Документация source of truth | Технический ответственный | PR #32 открыт из `izzhackt/evo-source-of-truth` в интеграционную ветку; основа документации — `86c4175d`, исправление фактов — `6f4b3391`, актуальный head показывает GitHub | 2026-07-12 | `gh pr view 32` |
...
1. Завершить review PR #32 и объединить его с `izzhacktcodex/waha-integration`.
2. Отдельно проверить всю интеграционную ветку, открыть PR в `main` и объединить
   его только после зелёных проверок.
3. Назначить людей, ответственных за бизнес, поступление, продажи,
   операционную и техническую работу.
4. Настроить DNS для двух канонических доменов и повторить HTTPS-проверку.
5. Сверить Git-коммит и образ каждого рабочего сервиса с GitHub и убрать остаточную
   зависимость CRM от `acadis_*`, если она всё ещё существует на VPS.
6. По отдельному контролируемому сценарию проверить реальные Supabase, WAHA,
   amoCRM и AI paths; записать доказательство без секретов и данных клиентов.
7. Проверить резервное копирование и реальное восстановление, а не только
