...
- Current merged checkpoint: `121db548b252eff9e4b79f62297aa27fe39e5c40`
- Active plan block:
  `EVO-P4B-AMOCRM-MAPPING-SELECTION-APPROVAL-PLAN-2026-08-04`
...
BW0, P3A-P3C, BW1-BW7, P2R0-P2R3 и P4A merged. PR #117
controller-merged P4A как
`121db548b252eff9e4b79f62297aa27fe39e5c40`; exact-main CI run
`30958119076` зелёный. Текущий plan block — P4B docs-only: он определяет
Admin-reviewed selection/approval одного immutable discovery version для
messaging use, без application code, migration, credentials/provider calls,
OAuth storage, identity sync, webhooks или writes. Staging, real provider
discovery, approved legal template, PDF/DOCX/e-sign, managed apply и customer
...
| P4B mapping selection/approval plan | Docs-only contract separates immutable discovery evidence from append-only Admin approval events and a deterministic current messaging selection behind the accepted `/whatsapp` seam | no implementation, migration, managed Supabase or provider proof in this amendment; later implementation remains blocked until controller merge and green exact-main CI |
...
- root `supabase/` — единственный repository migration source; P2A сохраняет
  ...
  discovery versions; P4B amendment не добавляет migration, а следующий
  implementation обязан заново подтвердить expected next-free 059;
...
Текущий gate — merge docs-only P4B contract. Он разрешает только отдельный
последующий implementation slice: current same-organization Admin выбирает и
одобряет одну P4A discovery version для `messaging`, решения остаются
append-only, а current selection выводится детерминированно. No-approved state
fail-closed для mapping-dependent behavior; новый admin/settings frontend не
создаётся, используется accepted `/whatsapp` seam. Persistent staging и
реальный amoCRM account discovery остаются честными blockers до credentials,
sanitized test account и отдельной авторизации. Identity/context sync,
webhook/outbox/reconciliation и canonical writes остаются следующими P4
slices; real WAHA/AI/ACK proof — P5, restore duties — P7. Production cutover
остаётся отдельным authorized event с bounded
