...
Дополнительная проверка ранее не охваченной ширины320 CSS px выполнена на
`48545eebe8c1d2250d882049ae403196906afa0b`: обычный Admissions,12 существующих
задач без срока, открытие и закрытие своей задачи. Viewport/scrollWidth320,
один main, карточка288px с отступом16px, без горизонтального переполнения.
Первоначальные ожидания точных подписей «Без срока»/«Сохранить» не совпали с
фактическим DOM («Без срока — 12», свёрнутое «Изменить задачу»); после чтения
DOM проверены реальные элементы. Бизнес-команды не отправлялись, схема и код
не менялись. Device override снят, QA-вкладка закрыта. Отдельная квитанция
`root217-320-reflow.json`; прежний24-table parity не переименовывается в новый прогон.
