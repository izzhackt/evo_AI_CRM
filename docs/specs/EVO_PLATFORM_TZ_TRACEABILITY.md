...
| 13.7 Visa and finance | 8 | 8 | 0 | 0 | 0 | 0 |
| 13.8 Tasks, notifications, reports, admin | 10 | 7 | 3 | 0 | 0 | 0 |
| 13.9 Student Portal | 6 | 5 | 1 | 0 | 0 | 0 |
| 13.10 OP/OZO business workflows | 20 | 19 | 0 | 0 | 1 | 0 |
| **Итого** | **110** | **88** | **14** | **5** | **1** | **2** |

Доля требований с доказательством в репозитории (`реализовано`): **80 %**.
...
| FR-072 | реализовано | миграция 043 | `tests/platform-admissions.test.mjs` |
| FR-073 | реализовано | `src/lib/platform-admissions-actions.ts` | `tests/e2e/sensitive-permissions.spec.ts` |
...
| ID | Статус | Реализация | Доказательство |
| --- | --- | --- | --- |
| FR-075 | реализовано | `addTaskAction`, `moveTaskAction`, `completeTaskAction` | `tests/platform-admissions.test.mjs` |
| FR-076 | реализовано | миграция 069 | `tests/platform-p6c-overdue-notifications.test.mjs` |
...
3. Для требований об административных действиях дополнительно проверено наличие
   экрана и вызова из приложения, а не только наличие RPC и теста базы.
...
Второго пробела масштаба FR-006 не найдено. Найдены два более узких уточнения:
FR-083 переведён в `частично`, у FR-081 уточнена граница видимой поверхности.
