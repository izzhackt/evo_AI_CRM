...
Partly superseded — see «Состояние 23.09.2026» below: the app icon (#912)
and the phone questionnaire/application status/invite flow (#908) have landed.
...
Read from `git log -- ios/`, the merged PRs and the repository tree on
23.09.2026. No new build, test or simulator run backs this section.

What landed on `main` after wave 8:

- #908 (wave 9a): questionnaire, application status and invite entry on the
  phone (`ApplicationWizardView`, `ApplicationStatusView`, `InviteEntryView`).
- #912 (wave 9b): catalog map, search and filters (`UniversitiesMapView`,
  `UniversityCatalogFilterPolicy`), the single-flight reload gate, an
  accessibility pass across screens and the app icon —
  `AppIcon.appiconset/AppIcon.png` (1024×1024), generated by
  `scripts/generate-app-icon.py`. The icon slot is no longer empty.
...
- #944, #946, #964, #967, #982, #993, #999: catalogue-intake decoders,
  ...
  review for single documents.
- #1000: program package workflow and history (`ProgramPackageView`,
  `ProgramPackageHistoryView`) on the #999 server contract.
- #980: web/iPhone email-confirmation signup (`ApplicationWizardView`,
  `SignInView`, intake models/policy/transfer and their tests).
...
- CI runs only on `ubuntu-latest` and neither compiles nor tests Swift; iOS
  ...
  #944–#1000 program/package models.
- No `PrivacyInfo.xcprivacy` privacy manifest exists in `ios/`.
- `DEVELOPMENT_TEAM` is unset in `project.yml` and the generated project
  (`CODE_SIGN_STYLE: Automatic`); `MARKETING_VERSION` is `0.1.0`.
- The bundle identifier `com.evoadmissions.app` remains provisional (see
  «Bundle identifier» above).
