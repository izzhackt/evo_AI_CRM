...
export function scanArtifactPrivacy(value, { spdx = false } = {}, path = []) {
  if (typeof value === "string") {
    if (CREDENTIAL.test(value) || UUID.test(value) || PRIVATE_PATH.test(value)) fail("artifact contains sensitive-looking content");
  ...
  if (Array.isArray(value)) {
    value.forEach((item, index) => scanArtifactPrivacy(item, { spdx }, [...path, index]));
    return;
  }
  if (value && typeof value === "object") {
    for (const [key, item] of Object.entries(value)) {
      scanArtifactPrivacy(key, { spdx: false }, [...path, key, "$key"]);
      scanArtifactPrivacy(item, { spdx }, [...path, key]);
    }
  }
}

function scanRawArtifactPrivacy(raw) {
  if (CREDENTIAL.test(raw) || UUID.test(raw) || PRIVATE_PATH.test(raw)) fail("artifact contains sensitive-looking raw content");
}

export function scanArtifactFile(path, { spdx = false } = {}) {
  const raw = readFileSync(path, "utf8");
  scanRawArtifactPrivacy(raw);
  if (path.endsWith(".json")) scanArtifactPrivacy(JSON.parse(raw), { spdx });
  else scanArtifactPrivacy(raw);
}
...
const schemaPath = new URL("../docs/schemas/p8u2-root-candidate.schema.json", import.meta.url);
...
function generateSbom(path, imageId) {
  ...
  scanArtifactPrivacy(value, { spdx: true });
...
export function verifyFinalRoot(source, parent, root, result) {
  ...
  for (const file of EVIDENCE_FILES) scanArtifactFile(join(root, file), { spdx: file === "sbom-crm.spdx.json" });
}

export function runP8U2({ sourceRoot }) {
  ...
  verifyOrbStack();
  const { parent: evidenceParent, root: evidenceRoot } = prepareEvidencePaths(source);
  if (dockerImageInventory().has(APPLICATION.tag)) fail("target image tag already exists");
  if (dockerContainerInventory().has(APPLICATION.container)) fail("smoke container already exists");

  const tempRoot = mkdtempSync(join(tmpdir(), "evo-p8u2-"));
  ...
  try {
    ...
    const sbomInfo = generateSbom(tempSbom, image.Id);
    assertEvidencePaths(source, evidenceParent, evidenceRoot);
    ...
    const smokeInfo = runSmoke(tempSmoke, image.Id, (id) => { ownedContainerId = id; });
    assertEvidencePaths(source, evidenceParent, evidenceRoot);
    copyEvidence(tempSmoke, join(evidenceRoot, "smoke-crm.log"));

    failureStep = "privacy";
    for (const file of ARTIFACT_FILES) {
      scanArtifactFile(join(evidenceRoot, file), { spdx: file === "sbom-crm.spdx.json" });
    }
...
async function main() {
  ...
  const { evidenceRoot, result } = runP8U2({ sourceRoot: args["source-root"] });
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  main().catch((error) => {
    process.stderr.write(`P8U2 failed: ${error.message}\n`);
    process.exitCode = 1;
  });
}
