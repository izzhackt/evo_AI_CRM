...
if (args.length !== 2 || args[0] !== "--application-root" || !args[1]) {
  process.stderr.write("Usage: npm run p8v2d:prepare -- --application-root <exact-clean-application-checkout>\n");
  process.exitCode = 2;
} else {
  try {
    ...
    process.stdout.write(`p8v2d_${result.result_code}\n`);
    process.exitCode = result.result_code === "preparation_verified" ? 0 : 2;
  } catch (error) {
    process.stderr.write(`p8v2d_failed:${error?.code ?? "operation_failed"}\n`);
    process.exitCode = 2;
  }
}
