...
const IMAGES = Object.freeze([
  Object.freeze({
    name: "main_crm",
    prefix: "evo-crm",
    tag: `evo-crm:${APPLICATION_COMMIT}-linux-amd64`,
    archive: `evo-crm-${APPLICATION_COMMIT}-linux-amd64.tar`,
  ...
  Object.freeze({
    name: "evo_inbox",
    prefix: "evo-inbox",
    tag: `evo-inbox:${APPLICATION_COMMIT}-linux-amd64`,
    archive: `evo-inbox-${APPLICATION_COMMIT}-linux-amd64.tar`,
  ...
  Object.freeze({
    ...
    tag: `evo-lead-agent:${APPLICATION_COMMIT}-linux-amd64`,
    archive: `evo-lead-agent-${APPLICATION_COMMIT}-linux-amd64.tar`,
...
function validateImages(images, required) {
  ...
  for (const [index, image] of images.entries()) {
    ...
    if (
      image.tag !== `${spec.prefix}:${APPLICATION_COMMIT}-linux-amd64` ||
...
export const P8V2 = Object.freeze({
  ...
  authorization: "PREPARE-P8V2-2026-08-18.P8V2.1",
  images: IMAGES,
...
export async function verifyP8V2FinalRoot(root) {
  ...
  if (portable.version !== "p8v2-portable-image-identity.v1" || portable.source_commit !== APPLICATION_COMMIT || portable.source_tree !== APPLICATION_TREE || JSON.stringify(portable.images) !== JSON.stringify(result.images)) fail("portable/result image identity mismatch");
