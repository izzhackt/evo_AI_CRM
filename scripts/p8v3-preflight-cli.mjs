...
try {
  ...
  writeP8V3Preflight(join(outputRoot, "p8v3h-preflight.json"), result);
  process.stdout.write("p8v3h_preflight_verified\n");
} catch (error) {
  process.stderr.write(`p8v3h_preflight_failed:${error?.code ?? "operation_failed"}\n`);
  process.exitCode = 2;
}
