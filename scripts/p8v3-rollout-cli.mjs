...
try {
  ...
  const output = join(outputRoot, "p8v3j-rollout-result.json");
  ...
  process.stdout.write(`p8v3j_${result.result_code}\n`);
  process.exitCode = result.result_code === "rollout_verified" ? 0 : 2;
} catch (error) {
  process.stderr.write(`p8v3j_failed:${error?.code ?? "operation_failed"}\n`);
  process.exitCode = 2;
}
