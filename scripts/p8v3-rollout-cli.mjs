...
try {
  ...
  const output = join(outputRoot, "p8v3e-rollout-result.json");
  ...
  process.stdout.write(`p8v3e_${result.result_code}\n`);
  process.exitCode = result.result_code === "rollout_verified" ? 0 : 2;
} catch (error) {
  process.stderr.write(`p8v3e_failed:${error?.code ?? "operation_failed"}\n`);
  process.exitCode = 2;
}
