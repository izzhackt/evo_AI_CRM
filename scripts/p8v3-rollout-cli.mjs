...
try {
  ...
  const output = join(outputRoot, "p8v3d-rollout-result.json");
  ...
  process.stdout.write(`p8v3d_${result.result_code}\n`);
  process.exitCode = result.result_code === "rollout_verified" ? 0 : 2;
} catch (error) {
  process.stderr.write(`p8v3d_failed:${error?.code ?? "operation_failed"}\n`);
  process.exitCode = 2;
}
