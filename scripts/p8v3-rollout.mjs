...
export const P8V3 = Object.freeze({
  ...
  authorization: "EXECUTE-P8V3K-2026-08-21.P8V3K.1",
  preflightAuthorization: "PREFLIGHT-P8V3K-2026-08-21.P8V3K.1",
  previousAuthorization: "EXECUTE-P8V3J-2026-08-21.P8V3J.1",
  legacyAuthorization: "EXECUTE-P8V3I-2026-08-21.P8V3I.1",
  earlierAuthorization: "EXECUTE-P8V3H-2026-08-21.P8V3H.1",
  historicalAuthorization: "EXECUTE-P8V3G-2026-08-21.P8V3G.1",
  earliestAuthorization: "EXECUTE-P8V3F-2026-08-20.P8V3F.1",
  initialAuthorization: "EXECUTE-P8V3E-2026-08-20.P8V3E.1",
...
const ACCEPTED_AUTHORIZATIONS = Object.freeze([
  P8V3.authorization,
  P8V3.previousAuthorization,
  P8V3.legacyAuthorization,
  P8V3.earlierAuthorization,
  P8V3.historicalAuthorization,
  P8V3.earliestAuthorization,
  P8V3.initialAuthorization,
]);
const KNOWLEDGE_REASON_AUTHORIZATIONS = Object.freeze(ACCEPTED_AUTHORIZATIONS.slice(0, 3));
const LEAD_FILE_AUTHORIZATIONS = Object.freeze(ACCEPTED_AUTHORIZATIONS.slice(0, 6));
...
function isExactRolledBackFailure(value) {
  const originalConfigurationFailure = value.failure?.step === "pre_state"
    ...
    && (KNOWLEDGE_REASON_AUTHORIZATIONS.includes(value.authorization?.id) ? value.configuration.lead_file_verified === null : !("lead_file_verified" in value.configuration))
  ...
  const reconciledMigrationFailure = KNOWLEDGE_REASON_AUTHORIZATIONS.includes(value.authorization?.id)
...
export function validateP8V3Result(value) {
  ...
  if (![
    ...
    "EXECUTE-P8V3D-2026-08-20.P8V3D.1",
    "EXECUTE-P8V3E-2026-08-20.P8V3E.1",
    P8V3.authorization,
    P8V3.previousAuthorization,
    P8V3.legacyAuthorization,
    P8V3.earlierAuthorization,
    ...
    P8V3.earliestAuthorization,
    P8V3.initialAuthorization,
  ].includes(value.authorization.id) || !UTC.test(value.authorization.started_at) || !UTC.test(value.authorization.expires_at) || Date.parse(value.authorization.expires_at) - Date.parse(value.authorization.started_at) !== P8V3.windowMs || !SHA64.test(value.authorization.preflight_sha256)) fail("authorization window drifted");
  ...
  if (noOpMigrationAuthorization && value.migration.status === "verified" && (value.migration.before_range !== "001-077" || value.migration.after_range !== "001-077" || value.migration.applied_versions.length !== 0 || value.effects.migration_applied !== 0)) fail("P8V3E-P8V3K reconciliation evidence drifted");
  if (value.failure !== null) {
    const requiresReason = KNOWLEDGE_REASON_AUTHORIZATIONS.includes(value.authorization.id) && ["client_import", "internal_import"].includes(value.failure.step);
  ...
  const expectedMigrationEffect = LEAD_FILE_AUTHORIZATIONS.includes(value.authorization.id) ? 0 : 1;
...
export async function runP8V3Rollout({ operations, authorization, preflight, preflightSha256, now = () => Date.now() }) {
  ...
  if (!SHA64.test(preflightSha256) || !preflight || preflight.application?.commit !== P8V3.applicationCommit || preflight.application?.tree !== P8V3.applicationTree || !UTC.test(preflight.generated_at) || !UTC.test(preflight.expires_at) || now() >= Date.parse(preflight.expires_at) || !Array.isArray(preflight.containers) || preflight.containers.length !== 5 || preflight.compose_validated !== true || preflight.prerequisites_verified !== true || preflight.migration?.range !== "001-077" || preflight.migration?.count !== 77 || !Array.isArray(preflight.archives) || preflight.archives.length !== 3 || preflight.crm_runtime_probe?.status !== "verified" || JSON.stringify(preflight.crm_runtime_probe?.backend) !== JSON.stringify({ version: "29.4.0", api: "1.54", os: "linux", arch: "amd64", driver: "io.containerd.snapshotter.v1" }) || !SHA64.test(preflight.crm_runtime_probe?.archive_sha256 ?? "") || !IMAGE_ID.test(preflight.crm_runtime_probe?.index ?? "") || !IMAGE_ID.test(preflight.crm_runtime_probe?.platform ?? "") || preflight.crm_runtime_probe?.runtime_image_id !== preflight.crm_runtime_probe?.index || !["absent", "exact_present"].includes(preflight.crm_runtime_probe?.source_prestate) || preflight.crm_runtime_probe?.compose_prestate !== "absent" || preflight.crm_runtime_probe?.nonce_prestate !== "absent" || preflight.crm_runtime_probe?.provider_container_image_id !== preflight.crm_runtime_probe?.index || !SHA64.test(preflight.crm_runtime_probe?.provider_container_id ?? "") || preflight.crm_runtime_probe?.cleanup_verified !== true || preflight.crm_runtime_probe?.production_unchanged !== true || preflight.gemini?.embedding_verified !== true || preflight.gemini?.draft_verified !== true || preflight.gemini?.retrieval_provider_verified !== true || preflight.gemini?.server_compose_verified !== true || !SHA64.test(preflight.importer?.sha256) || !Number.isSafeInteger(preflight.importer?.size) || preflight.importer.size < 1 || preflight.importer.verified !== true) fail("reviewed P8V3K preflight is missing, expired or drifted", "preflight_drift");
