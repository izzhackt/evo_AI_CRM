...
SQL
  ...
  # Migration 112 adds the one explicit main application and its university-
  # owned all-day deadline without weakening the status-event contract.
  if [[ "$(basename "$migration")" == 112_* ]]; then
    docker exec "$container_name" \
      psql -X -v ON_ERROR_STOP=1 -h 127.0.0.1 -U postgres -d "$test_database" \
  ...
  if [[ "$(basename "$migration")" == 134_* ]]; then
    docker exec "$container_name" \
      psql -X -v ON_ERROR_STOP=1 -h 127.0.0.1 -U postgres -d "$test_database" \
  ...
  if [[ "$(basename "$migration")" == 152_* ]]; then
    docker exec "$container_name" \
      psql -X -v ON_ERROR_STOP=1 -h 127.0.0.1 -U postgres -d "$test_database" \
  ...
  if [[ "$(basename "$migration")" == 154_* ]]; then
    docker exec "$container_name" \
      psql -X -v ON_ERROR_STOP=1 -h 127.0.0.1 -U postgres -d "$test_database" \
  ...
  # assignment working again for the Admin only, the requirements save for
  # document.manage holders, lead cabinet preparation for the Sales Manager on
