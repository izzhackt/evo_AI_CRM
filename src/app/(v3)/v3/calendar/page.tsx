...
export default async function CalendarPart({
  ...
  const [params, today, cases] = await Promise.all([
    searchParams,
    readToday(),
    // Список людей нужен форме заведения: задача принадлежит делу студента, и
    // «ничьей» её завести нельзя. Читается здесь — компонент ничего не грузит.
    readCaseOptions(),
  ]);
  ...
  const tasks = await readCalendarTasks(days[0], days[days.length - 1]);
