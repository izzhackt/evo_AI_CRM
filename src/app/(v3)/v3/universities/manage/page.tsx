...
import { universityUuid, withUniversityIntakeIds, type UniversityContent } from "@/lib/platform-university-catalog";
import { readStaffUniversities, readUniversityDrafts, readUniversityBatchSnapshot, reviewedUniversityTemplates } from "@/lib/v3/university-source";
...
export default async function UniversityManagePage({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  if (Object.keys(params).length > 1 || Object.entries(params).some(([key, value]) => !["draft", "edit", "identify", "template", "new", "batch"].includes(key) || typeof value !== "string")) notFound();
  ...
  const identifyId = params.identify === undefined ? null : universityUuid(params.identify) ?? notFound();
  if (params.new !== undefined && params.new !== "1") notFound();
  if (params.batch !== undefined && params.batch !== "1") notFound();
  ...
  if (params.template !== undefined || editId || identifyId || params.new === "1") {
