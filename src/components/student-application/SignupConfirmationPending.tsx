...
import { signupPendingMessageKey, type SignupPendingPresentation } from "@/lib/portal/signup-confirmation-presentation";

export function SignupConfirmationPending({ initial, locale, restored = false }: { initial: SignupResendResult; locale: Locale; restored?: boolean }) {
  const [presentation, action, busy] = useActionState(
    async (previous: SignupPendingPresentation, form: FormData): Promise<SignupPendingPresentation> => ({
      result: await resendStudentSignupConfirmationAction(previous.result, form),
      restored: false,
    }),
    { result: initial, restored },
  );
  const state = presentation.result;
  ...
  return <section aria-labelledby="signup-pending-title" className="rounded-card border border-border bg-surface p-5 sm:p-10">
    ...
    <p role="status" className="mt-4 text-fg-2">{strings[signupPendingMessageKey(presentation)]}</p>
