 ...
 * Ink, not red (Э3, 26.09.2026): the bars and dots carry magnitude, not a
 * problem or the page's main action, so they use the text colours.
 */
      ...
      {stages.map((stage, index) => (
        <li key={stage.name} className="relative grid min-h-12 grid-cols-[1rem_minmax(0,1fr)_auto] items-center gap-x-3 py-2">
          <span aria-hidden="true" className="relative flex h-full items-center justify-center">
            ...
            <span className={`relative size-2 rounded-full ${stage.value > 0 ? "bg-fg-2" : "border border-control-edge bg-surface"}`} />
          </span>
          ...
          <span aria-hidden="true" className="col-start-2 col-end-4 mt-2 h-1 overflow-hidden rounded-full bg-surface-2">
            <span className="block h-full rounded-full bg-fg-2" style={{ width: `${largest === 0 ? 0 : stage.value / largest * 100}%` }} />
          </span>
        </li>
      ))}
    </ol>
