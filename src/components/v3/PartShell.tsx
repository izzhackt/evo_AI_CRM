...
export function PartShell({
  ...
  count?: number | null;
  description?: string;
  /** Второстепенная строка под заголовком (`t-meta`). */
  meta?: React.ReactNode;
  action?: React.ReactNode;
  /** `data-testid` корня страницы — для проверки выпуска. */
  testId?: string;
  ...
  if (width === "board") {
    return (
      <main data-testid={testId} className="flex w-full min-w-0 flex-col px-4 pb-4 pt-5 sm:px-6 md:min-h-0 md:flex-1">
        <PageHeader title={title} count={count} description={description} meta={meta} action={action} />
