 ...
 * тихо: «Закрыт · причина · дата», «Вернуть в работу», одна строка о том,
 * почему нет контактов и истории, и на каком этапе лид остановился. Имя —
 * заголовок страницы, возврат — ссылка над ним (`PartShell back`). После
 * возврата страница перечитывается и снова показывает обычную карточку лида
 * на прежнем этапе.
 */
export function ClosedLeadView({
  ...
  const canReopen = row.canManage && !readOnly;
  return (
        ...
        <ClosedLine
          ...
          details={
            <p className="t-meta text-fg-2">
              {canReopen ? closureWords.lead.detailsAfterReopen : closureWords.lead.detailsOpenOnly}
            </p>
          }
        />
      </div>
