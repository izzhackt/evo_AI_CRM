...
export function AdmissionsRoutePanel({ workspace, playbooks, documents, studentName, children }: { workspace: AdmissionsWorkspace; playbooks: AdmissionsPlaybook[]; documents: readonly { id: string; name: string; applicationIds: readonly string[] }[]; studentName: string; children?: React.ReactNode }) {
  ...
  return <div className="space-y-5" data-testid="admissions-route">
    ...
    <section id="admissions-applications" className="rounded-card border border-border bg-surface p-4 sm:p-5"><h3 className="font-semibold text-fg">Партнёр, подача и решение</h3><p className="mt-2 text-sm leading-6 text-fg-2">Маршрут следует основной заявке. Остальные заявки не закрываются автоматически.</p>
    ...
    <details id="admissions-visa" className="rounded-card border border-border bg-surface p-4 sm:p-5"><summary className="min-h-11 cursor-pointer font-semibold text-fg">{current.direction === "MY" ? "Готовность к въезду · EMGS, eVAL и MDAC" : "Подтверждения визы"}</summary>
