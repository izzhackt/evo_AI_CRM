...
import { admissionsDirectoryHref, ATTENTION_LABELS, DIRECTION_LABELS, withDocsSection } from "./admissions-view";
  ...
  return <details className="min-w-0 rounded-card border border-border bg-surface" data-testid="v3-student-case-directory" open={initiallyOpen}>
    ...
    <div className="min-w-0 space-y-5 border-t border-border px-4 py-5 sm:px-5">
      ...
      <nav aria-label="Страницы каталога студентов" className="flex flex-wrap items-center justify-between gap-3">
        {params.cursor ? <Link className="inline-flex min-h-11 items-center text-sm font-medium text-fg-2 hover:text-fg" href={admissionsDirectoryHref(params, undefined, docsMode)}>← К началу</Link> : <span />}
        {directory.hasNext && directory.nextCursor ? <Link className="inline-flex min-h-11 items-center text-sm font-medium text-fg-2 hover:text-fg" href={admissionsDirectoryHref(params, directory.nextCursor, docsMode)} rel="next">Следующие записи →</Link> : null}
      </nav>
    </div>
  </details>;
}
