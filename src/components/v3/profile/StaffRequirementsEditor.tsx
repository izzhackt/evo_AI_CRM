...
function ItemFields({ item, index, count, context, used, disabled, update, move, remove, id }: {
  ...
  return <section id={id} tabIndex={-1} className="min-w-0 border-b border-border py-5">
    <div className="mb-3 flex flex-wrap items-center justify-between gap-2"><h5 className="min-w-0 break-words font-semibold text-fg">{index + 1}. {item.label || "Новое требование"}</h5>
  ...
  return <section aria-busy={loading || sending} className="min-w-0 space-y-4 border-t border-border pt-4">
    ...
    {base && draft && !readDenied ? <form noValidate onSubmit={event => { event.preventDefault(); void save(false); }}>
      ...
      <fieldset disabled={locked} className="min-w-0"><legend className="text-base font-semibold text-fg">Требования ({draft.items.length})</legend>
        ...
        <section className="mt-7 space-y-3"><h5 className="font-semibold text-fg">Прежние источники{unresolved ? ` · нужно решение: ${unresolved}` : ""}</h5><p className="max-w-2xl text-sm leading-6 text-fg-2">Учтите каждый источник. Исключение из этого списка не удаляет документ и не меняет настройки заявки.</p>
        ...
        <section id={targetId("review")} tabIndex={-1} className="mt-7 space-y-3"><h5 className="font-semibold text-fg">Перед сохранением</h5>{requirementChanges(base, draft).length ? <ul className="list-disc space-y-1 pl-5 text-sm text-fg-2">{requirementChanges(base, draft).map((change, index) => <li key={index}>{change}</li>)}</ul> : <p className="text-sm text-fg-2">Состав и документы остаются прежними. Подтверждение сохранится новой редакцией.</p>}<p className="max-w-2xl text-sm leading-6 text-fg-2">Подтверждение списка не одобряет файлы. При изменении требований проверьте, подходит ли ранее принятая версия документа.</p><div id={targetId("reason")} tabIndex={-1}><Field label="Основание подтверждения или изменений"><textarea className={areaCls} value={draft.changeReason} onChange={event => change({ ...draft, changeReason: event.target.value })} /></Field></div><label className="flex min-h-11 items-start gap-2 py-2 text-sm text-fg"><input type="checkbox" className="mt-0.5 size-5 shrink-0 accent-accent" checked={reviewed} onChange={event => setReviewed(event.target.checked)} />Я проверил(а) состав, материалы и решения по прежним источникам</label><button type="submit" className={btnCls} disabled={locked || loading}>{sending ? "Сохраняем…" : "Сохранить список"}</button></section>
      </fieldset>
    </form> : null}
  </section>;
}
