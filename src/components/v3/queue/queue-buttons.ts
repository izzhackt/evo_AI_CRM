...
export const QUEUE_CONFIRM = `inline-flex min-h-11 items-center justify-center gap-1.5 rounded-ctl border border-fg bg-fg px-4 t-label text-surface enabled:hover:border-fg-2 enabled:hover:bg-fg-2 ${DISABLED}`;

export const QUEUE_SECONDARY = `inline-flex min-h-11 items-center justify-center gap-1.5 rounded-ctl border border-control-edge bg-surface px-3 t-label text-fg-2 enabled:hover:bg-surface-2 enabled:hover:text-fg ${DISABLED}`;
