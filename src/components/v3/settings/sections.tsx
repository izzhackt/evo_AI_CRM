    ...
    <div className="flex flex-col gap-4">
      ...
      */}
      <Card title="Не управляется на этом экране">
        <ul>
          {[
            ["Приглашения", "Администратор управляет приглашениями и доступом в разделе «Сотрудники»."],
            ...
            ["Смена пароля", "Администратор отправляет восстановление из раздела «Сотрудники»; сотрудник задаёт пароль по своей ссылке."],
          ].map(([what, why]) => (
            <li
              key={what}
        ...
        <Note>
          ...
          список показывает, где доступны отдельные административные действия.
        </Note>
      </Card>
    </div>
  );
