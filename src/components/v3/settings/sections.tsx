  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card title="События" aside={<Pill>{entries.length}</Pill>}>
        ...
        <Note>
          Кто именно это сделал — <strong>неизвестно и не может быть известно</strong>: личных
          учётных записей в системе нет, есть три общих входа по ролям. Поэтому в журнале
          стоит роль, а не человек.
        </Note>
      </Card>

      <Card title="Второй журнал — платформенный" aside={<Pill>выключен</Pill>}>
        <p className="px-4 py-3 text-sm leading-6 text-fg-2">
          Журналов в системе два, и они в разных базах. Тот, что выше, — шина бизнес-событий.
          Второй, платформенный, умеет больше: 97 действий, 49 типов объектов, постраничная
          выдача и выгрузка в CSV с ограничением 31 день и 5000 строк. Его бэкенд написан
          целиком и не вызывается ни одной страницей — он выключен флагом.
        </p>
        <Note>
          <code className="font-mono">EVO_PLATFORM_P7A_AUDIT_ENABLED</code> · процедура
          <code className="font-mono"> search_audit_events</code> · выгрузка
          <code className="font-mono"> /api/platform-audit/export</code>. Даже включённый, он
          покажет актора как Staff / Service / System, а «что менялось» — как класс изменения,
          а не сравнение до и после.
        </Note>
      </Card>
    </div>
  );
...
export function AccessSection({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      */}
      <Card title="Чего в системе нет">
        <ul>
          {[
            ["Сотрудников", "Роль неотделима от пары логин-секрет. Личных учётных записей нет."],
            ["Приглашений", "Позвать нового человека нельзя: приглашать некуда."],
            ["Ключей API", "Таблицы ключей приложения не существует."],
            ["Списка сессий", "Отозвать чужой вход нельзя. Сессия живёт 30 минут и работой не продлевается."],
            ["Смены секрета", "Только правка окружения на сервере и перезапуск."],
          ].map(([what, why]) => (
            <li
              key={what}
        ...
        <Note>
          Отсюда же следует, что журнал действий не может назвать человека: записывать некого.
        </Note>
      </Card>
    </div>
  );
      ...
      <Card title="Правила загрузки">
        <ul>
          {[
            ...
            ["Где лежат", "На диске сервера, каталог задаётся при развёртывании"],
            ["Кто видит", "Загружать и скачивать — администратор и приёмная. Продажи документов не видят вовсе"],
          ].map(([k, v]) => (
            <li
              key={k}
              className="flex flex-wrap gap-x-3 gap-y-0.5 border-b border-border px-4 py-2.5 last:border-b-0"
        ...
        <Note>
          ...
          Антивируса и разбора содержимого нет, шифрования на уровне приложения тоже.
        </Note>
      </Card>
