...
export function TaskCasePicker({ initialCases, initialHasMore, selectedCase, onCaseChange, disabled = false }: Readonly<{
  ...
  const [selected, setSelected] = useState(selectedCase?.id ?? "");
  const selectedRef = useRef(selectedCase?.id ?? "");
  ...
  useLayoutEffect(() => {
    // Closing or locking the picker revokes pending searches before they can
    // change the case (and the consumer's draft context). Cleanup also covers unmount.
    sequence.current += 1;
  ...
  function search(more = false) {
    ...
    startTransition(async () => {
      try {
        ...
        if (!next.some((row) => row.id === selectedRef.current)) {
          const nextCaseId = "";
