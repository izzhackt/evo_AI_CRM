...
import { UNIVERSITY_COUNTRIES, UNIVERSITY_LEVELS, UNIVERSITY_LEVEL_LABELS, universityIntakeLabel, universityPublicUrl, type PublishedUniversity, type UniversityContent, type UniversityFilters, type UniversityPage } from "@/lib/platform-university-catalog";
...
function date(value: string | null) { return value ? new Intl.DateTimeFormat("ru", { day: "numeric", month: "long", year: "numeric", timeZone: "UTC" }).format(new Date(`${value}T12:00:00Z`)) : "Нужно уточнить"; }
...
function Notes({ text }: { text: string }) {
  return text.split(/(https:\/\/[^\s]+)/g).map((part, index) => {
    ...
    return universityPublicUrl(href) ? <span key={index}><a className="break-all text-accent-text underline underline-offset-4" href={href} target="_blank" rel="noopener noreferrer">{href}<span className="sr-only"> (в новой вкладке)</span></a>{part.slice(href.length)}</span> : part;
  });
}
