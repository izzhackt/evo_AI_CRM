...
export function UniversityPhoto({ content, large = false, compact = false }: { content: Pick<UniversityContent, "photoKey">; large?: boolean; compact?: boolean }) {
  ...
  const emptyClassName = `flex items-center justify-center rounded-card border border-border bg-surface-2 px-6 text-center text-sm text-fg-3 ${large ? "h-48" : compact ? "min-h-24 py-2" : "h-44"}`;
  if (!photo || src === null) return <div className={emptyClassName}>Проверенное фото кампуса пока не добавлено</div>;
  return <UniversityPhotoFrame src={src} alt={photo.caption} className="overflow-hidden rounded-card border border-border bg-surface" imageClassName={`w-full object-cover ${large ? "h-56 sm:h-80" : compact ? "h-24" : "h-44"}`} emptyClassName={emptyClassName} failedText="Не удалось загрузить фото кампуса">
    <figcaption className={`px-3 py-2 text-xs leading-5 text-fg-3${compact ? " [overflow-wrap:anywhere]" : ""}`}><p className="mb-1">{photo.caption}</p><a href={photo.sourceUrl} target="_blank" rel="noopener noreferrer" className="underline">Фото: {photo.author}</a> · <a href={photo.licenseUrl} target="_blank" rel="noopener noreferrer" className="underline">{photo.license}</a>. Кадрирование в карточке.</figcaption>
