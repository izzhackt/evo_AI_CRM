...
export const evoConversations = pgTable(
  "evo_conversations",
  {
    id: text("id").primaryKey(),
    leadId: text("lead_id")
    ...
    owningRole: text("owning_role").default("sales").notNull(),
    version: integer("version").default(1).notNull(),
    createdAt: timestamp("created_at", {
      withTimezone: true,
      mode: "date",
  ...
  (table) => [
    index("evo_conversations_lead_idx").on(table.leadId),
    unique("evo_conversations_external_unique").on(
      table.channel,
      table.externalConversationId,
    ),
    check(
      "evo_conversations_id_uuid_check",
      sql`${table.id} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'`,
    ),
    check(
      "evo_conversations_channel_check",
      sql`${table.channel} = 'whatsapp'`,
    ),
    check(
      "evo_conversations_external_id_check",
      sql`${table.externalConversationId} is null or char_length(btrim(${table.externalConversationId})) > 0`,
    ),
    check(
      "evo_conversations_status_check",
      sql`${table.status} in ('open', 'closed')`,
    ),
    check(
      "evo_conversations_owning_role_check",
      sql`${table.owningRole} in ('sales', 'admissions')`,
    ),
    check("evo_conversations_version_check", sql`${table.version} > 0`),
  ],
);

export const evoMessages = pgTable(
  "evo_messages",
  {
    id: text("id").primaryKey(),
  ...
  (table) => [
    unique("evo_messages_idempotency_unique").on(table.idempotencyKey),
    unique("evo_messages_external_unique").on(
      table.conversationId,
      table.externalMessageId,
    ),
    index("evo_messages_conversation_idx").on(table.conversationId),
    check(
      "evo_messages_id_uuid_check",
      sql`${table.id} ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'`,
    ),
    check(
      "evo_messages_direction_check",
      sql`${table.direction} in ('inbound', 'outbound')`,
    ),
    check(
      "evo_messages_external_id_check",
      sql`(${table.direction} = 'inbound' and ${table.externalMessageId} is not null and char_length(btrim(${table.externalMessageId})) > 0) or (${table.direction} = 'outbound' and (${table.externalMessageId} is null or char_length(btrim(${table.externalMessageId})) > 0))`,
    ),
    check(
      "evo_messages_author_role_check",
      sql`(${table.direction} = 'inbound' and ${table.authorRole} is null) or (${table.direction} = 'outbound' and ${table.authorRole} is not null and ${table.authorRole} in ('admin', 'sales', 'admissions'))`,
    ),
    check(
      "evo_messages_body_check",
      sql`char_length(btrim(${table.body})) > 0`,
    ),
    check(
      "evo_messages_correlation_check",
      sql`char_length(btrim(${table.correlationId})) > 0`,
    ),
    check(
      "evo_messages_idempotency_check",
