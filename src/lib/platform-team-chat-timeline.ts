import {
  ...
  type TeamChatChannelKey, type TeamChatMessage,
} from "./platform-team-chat.ts";

export type TeamChatTimelineQuery = Readonly<{ channel: TeamChatChannelKey } & (
  | { mode: "latest"; cursor?: "0"; messageId?: null }
  | { mode: "before" | "after"; cursor: string; messageId?: null }
  | { mode: "context"; cursor?: "0"; messageId: string }
)>;
export type TeamChatQuote = Readonly<{
  id: string; sequence: string; authorMembershipId: string; authorName: string;
  version: string; deletedAt: string | null; bodyPreview: string;
}>;
export type TeamChatTimelinePage = Readonly<{
  messages: readonly TeamChatMessage[]; quotes: readonly TeamChatQuote[];
...
export function teamChatTimelineValidQuery(value: unknown): value is TeamChatTimelineQuery {
  if (!record(value) || !isTeamChatChannel(value.channel)
    || Object.keys(value).some((key) => !["channel", "mode", "cursor", "messageId"].includes(key))) return false;
  if (value.mode === "before" || value.mode === "after") {
    return positiveCursor(value.cursor) && (value.messageId === undefined || value.messageId === null);
  }
  if (value.cursor !== undefined && value.cursor !== "0") return false;
  if (value.mode === "context") return teamChatUuid(value.messageId);
  return value.mode === "latest" && (value.messageId === undefined || value.messageId === null);
}
...
function quote(value: unknown): value is TeamChatQuote {
  return record(value) && teamChatUuid(value.id) && positiveCursor(value.sequence)
...
export function decodeTeamChatTimelinePage(value: unknown, query: TeamChatTimelineQuery): TeamChatTimelinePage | null {
  if (!teamChatTimelineValidQuery(query) || !record(value)
    || !Array.isArray(value.messages) || value.messages.length > 50 || !value.messages.every(message)
    || !Array.isArray(value.quotes) || value.quotes.length > 50 || !value.quotes.every(quote)
  ...
  const messages: TeamChatMessage[] = value.messages;
  ...
  for (const row of messages) {
    ...
    if (row.parentMessageId) parents.add(uuidKey(row.parentMessageId));
  }
  ...
  for (const item of quotes) {
    ...
    if (!parents.has(id) || seenQuotes.has(id)) return null;
    seenQuotes.add(id);
  ...
  if (seenQuotes.size !== parents.size) return null;
  return value as TeamChatTimelinePage;
}
