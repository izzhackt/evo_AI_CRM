...
export function decodeTeamChatChannels(value: unknown): readonly TeamChatChannel[] | null {
  ...
  for (const row of value) {
    ...
    let latestPreviewCreatedAt: string | null | undefined;
    if (Object.hasOwn(row, "latestPreviewCreatedAt")) {
      if (row.latestPreview === null) {
        if (row.latestPreviewCreatedAt !== null) return null;
        latestPreviewCreatedAt = null;
      } else {
        latestPreviewCreatedAt = normalizeTeamChatPreviewCreatedAt(row.latestPreviewCreatedAt);
        if (latestPreviewCreatedAt === null) return null;
      }
    }
    ...
    channels.push({
      ...
      ...(latestPreviewCreatedAt === undefined ? {} : { latestPreviewCreatedAt }),
    });
  }
  return channels;
}
...
export function acceptTeamChatChannels(
  ...
  for (const channel of incoming) {
    const previousChannel = previous.channels.find((item) => item.key === channel.key);
    const before = previousChannel?.latestPreview;
    const beforeTime = previousChannel?.latestPreviewCreatedAt;
    const { latestPreviewCreatedAt: incomingTime, ...metadata } = channel;
    let latestPreviewCreatedAt = incomingTime;
    ...
    if (before && latestPreview) {
      ...
      // Inputs have passed the strict decoder, so known times are canonical UTC.
      if (sameId && beforeTime !== undefined && incomingTime !== undefined
        && beforeTime !== incomingTime) return null;
      if (BigInt(before.sequence) > BigInt(latestPreview.sequence)
        || (sameId && BigInt(before.version) > BigInt(latestPreview.version))) {
        latestPreview = before;
        latestPreviewCreatedAt = sameId ? beforeTime ?? incomingTime : beforeTime;
      } else if (sameId) {
        latestPreviewCreatedAt = incomingTime ?? beforeTime;
      }
    } else if (before && latestPreview === null) {
      // Current product deletion preserves the latest row as a tombstone.
      latestPreview = before;
      latestPreviewCreatedAt = beforeTime;
    }
    channels.push({ ...metadata, latestPreview: latestPreview ? { ...latestPreview } : null,
      ...(latestPreviewCreatedAt === undefined ? {} : { latestPreviewCreatedAt }) });
  }
