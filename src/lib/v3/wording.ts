...
export const settingsStatusWords = {
  ...
  /** Проверка не запускалась: это не ошибка и не «Требует внимания». */
  notChecked: "не проверялось",
  /** Провайдер не настраивали: не тревога, а факт. */
  unused: "не используется",
  /** WhatsApp без данных о подключении к CRM. */
...
export function caseNextActionOutcome(value: string, cleared = false): string | null {
  const labels: Record<string, string> = {
    ...
    // Версия дела (admissions_version) растёт с каждой записью шага и, у дел с
    // playbook, со сменой основной заявки (137) — поэтому три причины, не одна.
    stale: "Шаг уже изменили: другой сотрудник, другая вкладка или правка дела. Обновите.",
