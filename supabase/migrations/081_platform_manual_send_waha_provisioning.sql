...
-- Add a supported, replay-safe service boundary for creating or rotating the
-- Supabase Vault secret used by the exact unified manual-send WAHA binding.

ALTER TABLE platform_private.manual_send_waha_runtime_bindings
  ADD COLUMN api_key_sha256 TEXT;

-- A previously provisioned disposable/staging binding can be carried forward
-- only when its Vault value still satisfies the runtime contract. Production
-- secrets are never embedded in this migration.
UPDATE platform_private.manual_send_waha_runtime_bindings AS binding
SET api_key_sha256 = pg_catalog.encode(
  pg_catalog.sha256(
...
WHERE secret.id = binding.api_key_secret_id
  AND secret.decrypted_secret IS NOT NULL
  AND secret.decrypted_secret = pg_catalog.btrim(
    secret.decrypted_secret,
    E' \t\n\r\f\v'
  )
  AND pg_catalog.octet_length(secret.decrypted_secret) BETWEEN 16 AND 4096
  AND pg_catalog.strpos(secret.decrypted_secret, pg_catalog.chr(10)) = 0
  AND pg_catalog.strpos(secret.decrypted_secret, pg_catalog.chr(13)) = 0;
...
BEGIN
  IF EXISTS (
    ...
    WHERE binding.api_key_sha256 IS NULL
  ) THEN
    RAISE EXCEPTION
      'Existing manual-send WAHA binding has no valid Vault key; repair it before applying migration 081'
      USING ERRCODE = '55000';
  END IF;
END
$migration_guard$;
...
ALTER TABLE platform_private.manual_send_waha_runtime_bindings
  ALTER COLUMN api_key_sha256 SET NOT NULL;

ALTER TABLE platform_private.manual_send_waha_runtime_bindings
  ADD CONSTRAINT manual_send_waha_runtime_bindings_api_key_sha256_check
  CHECK (api_key_sha256 ~ '^[0-9a-f]{64}$');
...
BEGIN
  ...
  resolved_reason_code := CASE
    ...
    WHEN binding_row.waha_session_name <> 'evo-inbox'
      OR binding_row.waha_base_url <> 'http://evo-crm-waha:3000'
...
BEGIN
  ...
  IF FOUND THEN
    ...
    IF NOT existing_binding.enabled
      ...
      OR existing_binding.waha_base_url <> 'http://evo-crm-waha:3000'
...
-- The private table and both RPCs remain unreachable to browser roles. A
-- backend can configure or inspect the binding only with service-role authority.
REVOKE ALL ON TABLE
