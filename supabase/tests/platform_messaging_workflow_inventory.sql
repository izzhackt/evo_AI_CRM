...
DECLARE
  public_p2f_tables CONSTANT TEXT[] := ARRAY[
    ...
    'ai_draft_request_knowledge_selections',
    'ai_draft_requests',
    'ai_drafts',
  ...
  private_p2f_tables CONSTANT TEXT[] := ARRAY[
    'messaging_integration_health_events',
...
BEGIN
  ...
  IF enum_range(NULL::platform.communication_queue)::TEXT[]
    ...
    OR enum_range(NULL::platform.messaging_integration_target)::TEXT[]
      IS DISTINCT FROM ARRAY['ai', 'waha']::TEXT[]
    OR enum_range(NULL::platform.messaging_integration_readiness)::TEXT[]
      IS DISTINCT FROM ARRAY[
        'ready',
        'blocked',
    ...
    OR enum_range(NULL::platform.messaging_integration_evidence_kind)::TEXT[]
      IS DISTINCT FROM ARRAY[
        'configuration_check',
        'local_non_provider',
        'provider_observed'
...
DECLARE
  expected_policies CONSTANT TEXT[] := ARRAY[
    ...
    'ai_draft_request_knowledge_selections:ai_draft_request_knowledge_selections_full_read',
  ...
  FOR projection IN
    ...
    FROM (
      VALUES
        ...
        (
          'platform.staff_conversation_workflow(uuid,uuid)'::TEXT,
          ARRAY[
            'conversation_id:uuid',
            ...
            'latest_inbound_created_at:timestamp with time zone',
            ...
            'latest_ai_draft_created_at:timestamp with time zone',
            'latest_ai_draft_updated_at:timestamp with time zone',
            'latest_ai_draft_failure_outcome:text',
            'latest_manual_send_authorization_id:uuid',
            'latest_manual_send_final_text:text',
            'latest_manual_send_authorized_at:timestamp with time zone',
            ...
            'latest_outbox_available_at:timestamp with time zone',
            'latest_outbox_updated_at:timestamp with time zone',
            ...
            'latest_outbox_observed_at:timestamp with time zone',
            'latest_audit_action:text',
            'latest_audit_at:timestamp with time zone'
          ]::TEXT[],
          'ORDER BY event.observed_at DESC, event.id DESC'::TEXT
        ),
        (
