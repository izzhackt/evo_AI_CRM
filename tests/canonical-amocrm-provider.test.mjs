...
test("canonical amoCRM ensures one exact lead tag and accepts the existing same-name tag response", async () => {
  const { tokenFilePath } = await makePrivateTokenFile();
  const calls = [];
  const provider = createCanonicalAmoCrmWriteProvider(readyConfig(tokenFilePath), {
    fetch: async (url, init) => {
      ...
      return jsonResponse(
        {
          ...
          _embedded: {
            tags: [
              {
                id: 481516,
                name: "EVO V2 Admissions",
                request_id: "ensure-admissions-tag-1",
              },
            ],
          },
        },
        { headers: { "x-request-id": "provider-request-tag-1" } },
      );
    },
  });

  const result = await provider.ensureLeadTag({
    requestId: "ensure-admissions-tag-1",
    name: "EVO V2 Admissions",
  });
  ...
  assert.equal(
    calls[0].url,
    "https://evoadmissions.amocrm.ru/api/v4/leads/tags",
  );
  assert.equal(calls[0].init.method, "POST");
  assert.equal(calls[0].init.redirect, "error");
  assert.equal(calls[0].init.headers.Authorization, `Bearer ${SECRET_ACCESS}`);
  ...
  assert.deepEqual(result, {
    ...
    response: {
      status: 200,
      providerRequestId: "provider-request-tag-1",
    },
  });
});

test("canonical amoCRM rejects a lead-tag response outside the documented collection envelope", async () => {
  const { tokenFilePath } = await makePrivateTokenFile();
  const provider = createCanonicalAmoCrmWriteProvider(readyConfig(tokenFilePath), {
    fetch: async () =>
      jsonResponse({
        id: 481516,
        name: "EVO V2 Sales",
        request_id: "ensure-sales-tag-1",
        diagnostic: SECRET_ACCESS,
      }),
  });

  await assert.rejects(
    provider.ensureLeadTag({
      requestId: "ensure-sales-tag-1",
      name: "EVO V2 Sales",
    }),
    (error) => {
...
test("canonical amoCRM rejects unsafe lead-tag names and request IDs before dispatch", async () => {
  const { tokenFilePath } = await makePrivateTokenFile();
  let callCount = 0;
  const provider = createCanonicalAmoCrmWriteProvider(readyConfig(tokenFilePath), {
    fetch: async () => {
  ...
  for (const input of [
    { requestId: "safe-id", name: " EVO V2 Sales" },
    { requestId: "safe-id", name: "EVO\nV2 Sales" },
    { requestId: "safe-id", name: `EVO ${"x".repeat(256)}` },
    { requestId: "safe-id", name: "EVO \ud800 V2 Sales" },
    { requestId: "not safe", name: "EVO V2 Sales" },
  ]) {
    assert.throws(
      () => provider.prepareEnsureLeadTag(input),
      (error) =>
        error instanceof CanonicalAmoCrmProviderError &&
...
test("canonical amoCRM freezes the exact lead-tag mutation and dispatches it only once", async () => {
  const { tokenFilePath } = await makePrivateTokenFile();
  let callCount = 0;
  let sentBody = null;
  const provider = createCanonicalAmoCrmWriteProvider(readyConfig(tokenFilePath), {
          ...
          tags: [
            {
              id: 481517,
              name: "EVO V2 Sales",
              request_id: "ensure-sales-tag-prepared",
            },
          ],
        },
      });
  ...
  const prepared = provider.prepareEnsureLeadTag({
    requestId: "ensure-sales-tag-prepared",
    name: "EVO V2 Sales",
  });
  ...
  assert.equal(
    prepared.bodyJson,
    '[{"name":"EVO V2 Sales","request_id":"ensure-sales-tag-prepared"}]',
  );
  assert.equal(
    prepared.bodySha256,
    "43a44ad3b3c7232bf0a84b552e3d5dfc6b591de48cc340de04295640ef360b50",
  );
  assert.equal(
    prepared.requestSha256,
    "0ee147caaac86a35545e23db0eec8ba55512372b5c95f88259de6746ed9ee7f3",
  );
  assert.equal(JSON.stringify(prepared).includes(SECRET_ACCESS), false);
  assert.equal(JSON.stringify(prepared).includes(SECRET_CLIENT), false);
...
test("canonical amoCRM rejects malformed, mismatched and ambiguous lead-tag responses", async () => {
  const { tokenFilePath } = await makePrivateTokenFile();
  const exactTag = {
    id: 481518,
    name: "EVO V2 Admissions",
    request_id: "ensure-admissions-tag-response",
  };
  const responses = [
    null,
    { _total_items: 1, _embedded: { tags: [] } },
    {
      _total_items: 2,
      _embedded: { tags: [exactTag, { ...exactTag, id: 481519 }] },
    },
    { _total_items: 1, _embedded: { tags: [{ ...exactTag, id: 0 }] } },
    {
      _total_items: 1,
      _embedded: { tags: [{ ...exactTag, id: "481518" }] },
    },
    {
      _total_items: 1,
      _embedded: { tags: [{ ...exactTag, name: "EVO V2 Sales" }] },
    },
    {
      _total_items: 1,
      _embedded: {
        tags: [{ ...exactTag, request_id: "different-request" }],
      },
    },
    { _embedded: { tags: [exactTag] } },
  ];

  for (const response of responses) {
    let callCount = 0;
    const provider = createCanonicalAmoCrmWriteProvider(
      readyConfig(tokenFilePath),
      {
        ...virtualClock(),
        fetch: async () => {
          callCount += 1;
          return jsonResponse(
            response === null
              ? null
              : { ...response, diagnostic: SECRET_REFRESH },
          );
        },
      },
    );
    ...
    await assert.rejects(
      provider.ensureLeadTag({
        requestId: "ensure-admissions-tag-response",
        name: "EVO V2 Admissions",
      }),
      (error) => {
        assert.ok(error instanceof CanonicalAmoCrmMutationError);
        ...
        assert.equal(error.message.includes(SECRET_REFRESH), false);
        assert.equal(JSON.stringify(error).includes(SECRET_REFRESH), false);
        return true;
      },
    );
    assert.equal(callCount, 1);
...
test("canonical amoCRM never refreshes or replays a lead-tag mutation after 401", async () => {
  const { tokenFilePath } = await makePrivateTokenFile();
  const calls = [];
  const provider = createCanonicalAmoCrmWriteProvider(readyConfig(tokenFilePath), {
    ...virtualClock(),
    fetch: async (url, init) => {
      calls.push({ url, init });
      return jsonResponse(
        { title: `Unauthorized ${SECRET_ACCESS}` },
        { status: 401 },
      );
    },
  });
  ...
  await assert.rejects(
    provider.ensureLeadTag({
      requestId: "ensure-sales-tag-unauthorized",
      name: "EVO V2 Sales",
    }),
    (error) => {
      assert.ok(error instanceof CanonicalAmoCrmMutationError);
  ...
  assert.equal(
    calls.some(({ url }) => String(url).endsWith("/oauth2/access_token")),
    false,
  );
});
