...
test("CI Node suite runs the former security and unit surface once", () => {
  ...
  assert.equal(plan.occurrenceCount, 328);
  assert.equal(plan.uniqueFileCount, 189);
  ...
  assert.equal(bounded.files.length, 158);
...
test("local unit command preserves its full logical surface without hidden hooks", () => {
  ...
  assert.equal(plan.occurrenceCount, 226);
  assert.equal(plan.uniqueFileCount, 184);
