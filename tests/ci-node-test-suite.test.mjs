...
test("CI Node suite runs the former security and unit surface once", () => {
  ...
  assert.equal(plan.occurrenceCount, 276);
  assert.equal(plan.uniqueFileCount, 137);
  ...
  assert.equal(bounded.files.length, 115);
...
test("local unit command preserves its full logical surface without hidden hooks", () => {
  ...
  assert.equal(plan.occurrenceCount, 174);
  assert.equal(plan.uniqueFileCount, 132);
