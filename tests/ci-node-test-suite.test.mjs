...
test("CI Node suite runs the former security and unit surface once", () => {
  ...
  assert.equal(plan.occurrenceCount, 304);
  ...
  assert.equal(bounded.files.length, 137);
  for (const requiredD3Test of ["tests/document-recognition.test.mjs", "tests/gemini-document-recognition.test.mjs"]) {
    assert.ok(bounded.files.includes(requiredD3Test), requiredD3Test);
  }
...
test("local unit command preserves its full logical surface without hidden hooks", () => {
  ...
  assert.equal(plan.occurrenceCount, 202);
  assert.equal(plan.uniqueFileCount, 160);
  assert.equal(plan.duplicateCount, 42);
  assert.ok(plan.files.includes("tests/staff-auth-failure.test.mjs"));
  for (const requiredD3Test of ["tests/document-recognition.test.mjs", "tests/gemini-document-recognition.test.mjs"]) {
    assert.ok(plan.files.includes(requiredD3Test), requiredD3Test);
  }
});
