...
test("CI Node suite runs the former security and unit surface once", () => {
  ...
  assert.equal(plan.occurrenceCount, 354); // + PORT-9a, + PORT-9d (tests/university-photo-storage.test.mjs), + «Студенты» UI (tests/v3-students-queue-ui.test.mjs, ex-facets), + «Задачи» queue (tests/v3-tasks-queue.test.mjs), + «Студенты» queue backend (tests/v3-students-queue.test.mjs), + boards (tests/v3-boards.test.mjs), + «Сообщения» order (tests/v3-case-chat-order.test.mjs), + new look preview (tests/v3-look-preview.test.mjs), + truthful state (tests/v3-truthful-state.test.mjs)
  // Один assert на пин: задвоенная строка была случайной (дедуплицирована в PORT-5c).
  assert.equal(plan.uniqueFileCount, 215); // + PORT-9a, + PORT-9d, + «Студенты» UI (ex-facets), + «Задачи» queue, + «Студенты» queue backend, + boards, + «Сообщения» order, + new look preview, + truthful state
  ...
  assert.equal(bounded.files.length, 183); // + PORT-9a, + PORT-9d (tests/university-photo-storage.test.mjs), + «Студенты» facets, + «Задачи» queue, + «Студенты» queue backend, + boards, + «Сообщения» order, + new look preview, + truthful state
...
test("local unit command preserves its full logical surface without hidden hooks", () => {
  ...
  assert.equal(plan.occurrenceCount, 252); // + PORT-9a, + PORT-9d (tests/university-photo-storage.test.mjs), + «Студенты» facets, + «Задачи» queue, + «Студенты» queue backend, + boards, + «Сообщения» order, + new look preview, + truthful state
  // Один assert на пин: задвоенная строка была случайной (дедуплицирована в PORT-5c).
  assert.equal(plan.uniqueFileCount, 210); // + PORT-9a, + PORT-9d, + «Студенты» facets, + «Задачи» queue, + «Студенты» queue backend, + boards, + «Сообщения» order, + new look preview, + truthful state
