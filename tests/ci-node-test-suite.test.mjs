...
test("CI Node suite runs the former security and unit surface once", () => {
  ...
  assert.equal(bounded.files.length, 189); // + PORT-9a, + PORT-9d (tests/university-photo-storage.test.mjs), + «Студенты» facets, + «Задачи» queue, + «Студенты» queue backend, + boards, + «Сообщения» order, + new look preview, + truthful state, + access by permissions 244, + case work, + new look shell (Э1.2), + «Сегодня», + closure 246 (tests/v3-lead-case-closure.test.mjs), + access owner defaults 248 (tests/platform-access-owner-defaults.test.mjs)
...
test("local unit command preserves its full logical surface without hidden hooks", () => {
  ...
  assert.equal(plan.occurrenceCount, 259); // + PORT-9a, + PORT-9d (tests/university-photo-storage.test.mjs), + «Студенты» facets, + «Задачи» queue, + «Студенты» queue backend, + boards, + «Сообщения» order, + new look preview, + truthful state, + access by permissions 244, + case work, + new look shell (Э1.2), + «Сегодня», + closure 246 (tests/v3-lead-case-closure.test.mjs), + honest numbers 247 (tests/v3-honest-numbers.test.mjs), + access owner defaults 248 (tests/platform-access-owner-defaults.test.mjs)
  // Один assert на пин: задвоенная строка была случайной (дедуплицирована в PORT-5c).
  assert.equal(plan.uniqueFileCount, 217); // + PORT-9a, + PORT-9d, + «Студенты» facets, + «Задачи» queue, + «Студенты» queue backend, + boards, + «Сообщения» order, + new look preview, + truthful state, + access by permissions 244, + case work, + new look shell (Э1.2), + «Сегодня», + closure 246 (tests/v3-lead-case-closure.test.mjs), + honest numbers 247 (tests/v3-honest-numbers.test.mjs), + access owner defaults 248 (tests/platform-access-owner-defaults.test.mjs)
