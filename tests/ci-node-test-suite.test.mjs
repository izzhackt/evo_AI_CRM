...
test("CI Node suite runs the former security and unit surface once", () => {
  ...
  assert.equal(plan.occurrenceCount, 360); // + PORT-9a, + PORT-9d (tests/university-photo-storage.test.mjs), + «Студенты» UI (tests/v3-students-queue-ui.test.mjs, ex-facets), + «Задачи» queue (tests/v3-tasks-queue.test.mjs), + «Студенты» queue backend (tests/v3-students-queue.test.mjs), + boards (tests/v3-boards.test.mjs), + «Сообщения» order (tests/v3-case-chat-order.test.mjs), + new look preview (tests/v3-look-preview.test.mjs), + truthful state (tests/v3-truthful-state.test.mjs), + access by permissions 244 (tests/platform-access-by-permissions.test.mjs), + case work (tests/v3-case-work.test.mjs), + new look shell Э1.2 (tests/v3-shell-next.test.mjs), + «Сегодня» (tests/v3-today-queue.test.mjs), + shared blocks Э1.3 (tests/v3-blocks.test.mjs), + admissions board harness (tests/v3-admissions-pipeline.test.mjs)
  // Один assert на пин: задвоенная строка была случайной (дедуплицирована в PORT-5c).
  assert.equal(plan.uniqueFileCount, 221); // + PORT-9a, + PORT-9d, + «Студенты» UI (ex-facets), + «Задачи» queue, + «Студенты» queue backend, + boards, + «Сообщения» order, + new look preview, + truthful state, + access by permissions 244, + case work, + new look shell (Э1.2), + «Сегодня», + shared blocks (Э1.3), + admissions board harness
  ...
  assert.equal(bounded.files.length, 189); // + PORT-9a, + PORT-9d (tests/university-photo-storage.test.mjs), + «Студенты» facets, + «Задачи» queue, + «Студенты» queue backend, + boards, + «Сообщения» order, + new look preview, + truthful state, + access by permissions 244, + case work, + new look shell (Э1.2), + «Сегодня», + shared blocks (Э1.3), + admissions board harness
...
test("local unit command preserves its full logical surface without hidden hooks", () => {
  ...
  // Один assert на пин: задвоенная строка была случайной (дедуплицирована в PORT-5c).
  assert.equal(plan.uniqueFileCount, 216); // + PORT-9a, + PORT-9d, + «Студенты» facets, + «Задачи» queue, + «Студенты» queue backend, + boards, + «Сообщения» order, + new look preview, + truthful state, + access by permissions 244, + case work, + new look shell (Э1.2), + «Сегодня», + shared blocks (Э1.3), + admissions board harness
