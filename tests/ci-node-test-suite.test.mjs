...
test("CI Node suite runs the former security and unit surface once", () => {
  ...
  assert.equal(plan.occurrenceCount, 347); // + PORT-9a, + PORT-9d (tests/university-photo-storage.test.mjs)
  // Один assert на пин: задвоенная строка была случайной (дедуплицирована в PORT-5c).
  assert.equal(plan.uniqueFileCount, 208); // + PORT-9a, + PORT-9d
  ...
  assert.equal(bounded.files.length, 176); // + PORT-9a, + PORT-9d (tests/university-photo-storage.test.mjs)
...
test("local unit command preserves its full logical surface without hidden hooks", () => {
  ...
  assert.equal(plan.occurrenceCount, 245); // + PORT-9a, + PORT-9d (tests/university-photo-storage.test.mjs)
  // Один assert на пин: задвоенная строка была случайной (дедуплицирована в PORT-5c).
  assert.equal(plan.uniqueFileCount, 203); // + PORT-9a, + PORT-9d
