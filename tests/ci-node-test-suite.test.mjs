...
test("CI Node suite runs the former security and unit surface once", () => {
  ...
  assert.equal(plan.occurrenceCount, 325);
  assert.equal(plan.uniqueFileCount, 186);
  ...
  assert.equal(bounded.files.length, 155);
...
test("local unit command preserves its full logical surface without hidden hooks", () => {
  ...
  assert.equal(plan.occurrenceCount, 223);
  assert.equal(plan.uniqueFileCount, 181);
