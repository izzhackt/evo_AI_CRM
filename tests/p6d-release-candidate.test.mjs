...
test("CI and the exact-SHA gate require only the current root successor", async () => {
  ...
  assert.match(workflow, /name: Audit production dependencies[\s\S]*npm_config_fetch_retries: "0"[\s\S]*npm_config_fetch_timeout: "60000"/u);
  assert.match(workflow, /name: Audit development dependencies against the temporary allowlist[\s\S]*npm_config_fetch_retries: "0"[\s\S]*npm_config_fetch_timeout: "60000"/u);
