...
function identity() {
  return {
    schema_version: 1,
    candidate_commit: P8D4_CANDIDATE,
    target_platform: { os: "linux", architecture: "amd64", variant: "" },
    images: p8d4Specs.map((spec) => ({
      name: spec.name,
      tag: `${spec.prefix}:${P8D4_CANDIDATE}-linux-amd64`,
      source_commit: P8D4_CANDIDATE,
...
test("P8D4 portable schema pins the ordered d565 image identities", () => {
...
test("shared portable verifier uses the explicit candidate instead of the historical default", () => {
  ...
  const descriptor = {
    ...
    annotations: {
      ...
      "org.opencontainers.image.ref.name": `${P8D4_CANDIDATE}-linux-amd64`,
    },
  };
  const metadata = {
    ...
    config: {
      ...
      architecture: "amd64",
      config: { Labels: {
        "org.opencontainers.image.revision": P8D4_CANDIDATE,
