...
const root = process.env.EVO_P8D4F_EVIDENCE_ROOT;
if (!root) throw new Error("EVO_P8D4F_EVIDENCE_ROOT must name the retained real P8D4F evidence root");
...
test("real P8D4F archives, schema, modes and collection hashes are exact", () => {
  ...
  for (const spec of p8d4fSpecs) {
    ...
    assert.throws(() => validatePortableMetadata({ ...changedTag, spec, tag, candidate: P8D4F_CANDIDATE }), /archive tag mismatch/);
    ...
    assert.throws(() => validatePortableMetadata({ ...changedRevision, spec, tag, candidate: P8D4F_CANDIDATE }), /archive source mismatch/);
  }

  const temporary = mkdtempSync(join(tmpdir(), "evo-p8d4f-real-"));
  try {
    ...
    assert.throws(() => verifyPortableArchive(archive, spec, tag, P8D4F_CANDIDATE), /unexpected tar entries/);
    rmSync(archive);
    copyFileSync(join(root, spec.archive), archive);
    symlinkSync("unexpected.txt", join(temporary, "unsafe-link"));
    execFileSync("tar", ["-rf", archive, "-C", temporary, "unsafe-link"]);
    assert.throws(() => verifyPortableArchive(archive, spec, tag, P8D4F_CANDIDATE), /unsafe tar entry type/);
  } finally {
