...
test("malformed docker run output still removes the exact newly-created smoke container", () => {
  ...
  const name = `evo-p8v2-smoke-main-crm-${P8V2.applicationCommit.slice(0, 12)}`;
  const removed = [];
  let inventoryCall = 0;
  let ownerNonce = null;
  const docker = (args) => {
    ...
    if (args[0] === "container" && args[1] === "inspect") return { status: 0, stdout: JSON.stringify([{ Id: containerId, Image: IMAGE_ID, Config: { Labels: { "evo.p8v2.owner": ownerNonce } } }]), stderr: "" };
...
test("failed smoke run never removes a foreign container that races onto the reserved name", () => {
  ...
  const docker = (args) => {
    ...
    if (args[0] === "container" && args[1] === "inspect") return { status: 0, stdout: JSON.stringify([{ Id: foreignId, Image: IMAGE_ID, Config: { Labels: { "evo.p8v2.owner": "foreign" } } }]), stderr: "" };
    if (args[0] === "container" && args[1] === "rm") { removed.push(args.at(-1)); return { status: 0, stdout: "", stderr: "" }; }
    throw new Error(`unexpected docker call: ${args.join(" ")}`);
  };
...
test("candidate identity requires immutable ID, exact labels and an empty variant", () => {
  ...
  const inspect = {
    ...
    Config: { Labels: {
      ...
      "org.opencontainers.image.version": "p8v2-0f1454d0-20260818",
    } },
  };
...
test("knowledge builds remove every UUID-bearing root even when the second build fails", async () => {
  ...
  const operations = await createP8V2Operations({
    ...
    environment: {
      EVO_P8V2_AUTHORIZATION: P8V2.authorization,
...
test("an existing symlinked evidence parent is rejected before chmod or writes", async () => {
  ...
  const operations = await createP8V2Operations({
    ...
    environment: { EVO_P8V2_AUTHORIZATION: P8V2.authorization },
