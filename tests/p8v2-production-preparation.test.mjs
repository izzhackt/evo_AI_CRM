...
function verifiedImage(name, prefix) {
  return {
    name,
    tag: `${prefix}:${P8V2.applicationCommit}-linux-amd64`,
    archive: `${prefix}-${P8V2.applicationCommit}-linux-amd64.tar`,
    image_id: IMAGE_ID,
...
test("evidence publication verifies exact six-artifact index and rejects drift or privacy leaks", async () => {
  ...
  for (const file of artifacts) {
    ...
    let retained = null;
    if (file === "portable-image-identity.json") retained = { version: "p8v2-portable-image-identity.v1", source_commit: P8V2.applicationCommit, source_tree: P8V2.applicationTree, images: value.images };
